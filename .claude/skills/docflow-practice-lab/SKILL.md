...
3. From the selected DocFlow project workspace, inspect the task:
   ...
   ```bash
   python3 .claude/skills/docflow-practice-lab/scripts/job.py inspect --job-id <task-id>
   ```
...
10. After success or a controlled stop, redact the one-time task:
    ...
    ```bash
    python3 .claude/skills/docflow-practice-lab/scripts/job.py complete --job-id <task-id>
    ```
