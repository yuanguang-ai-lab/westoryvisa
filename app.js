...
function apiCompatibilityMessage() {
  ...
  return "当前地址没有连接到 DocFlow 后端。请重新运行“启动完整版本.command”，并使用它自动打开的网页地址。";
}
...
function syncApplication(application) {
  ...
  return fetch(`${API_BASE}/cases/${encodeURIComponent(application.id)}`, {
    method: "PUT",
    ...
    : payload.error
      ...
      : `
        <div class="ocr-preview-summary">
          <span>${escapeHtml(payload.parserName || "Docling Serve / RapidOCR")}</span>
  ...
  return `
    <aside class="sidebar">
      <button class="brand" type="button" onclick="goBack()" aria-label="返回上一页">
        <div class="brand-mark" lang="en">DF</div>
        <div>
        <div class="brand-title" lang="en">DocFlow DS-160</div>
      ...
      <section class="auth-panel">
        <div class="auth-copy">
          <div class="brand-line">
            ...
            <span lang="en">DocFlow DS-160</span>
          </div>
        ...
        <div class="auth-visual" aria-label="DocFlow DS-160 工作台预览">
...
async function sendRegistrationEmailCode() {
  ...
  showAuthError("");
  try {
    const response = await fetch(`${API_BASE}/email-verification/send`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function submitAuthForm(event) {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE}/${isRegister ? "register" : "login"}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  return `
    <section class="product-sections">
      <div class="product-section intro-band">
        ...
        <p>DocFlow DS-160 是给签证中介内部使用的资料整理与初稿生成工具。它帮助团队把客户材料整理成可复核的 DS-160 填写建议，而不是替客户提交申请。</p>
      </div>
  ...
  container.innerHTML = `
    <div class="topbar">
      ...
      <div class="topbar-actions">
        ...
        <a class="btn secondary" href="/product.html" target="_blank" rel="noopener">查看产品页</a>
...
async function refreshOcrServiceStatus() {
  ...
  try {
    const response = await fetch(`${API_BASE}/ocr/health`);
  ...
  if (serviceButton) {
    ...
    serviceButton.textContent = state.ocrService.starting ? "正在启动…" : "启动扫描服务";
  }
  const application = getActiveApplication();
  ...
  if (application && startButton) {
    ...
    startButton.title = !hasUploadedFile
      ? "请先上传至少一份客户材料"
      : (hasActiveUpload ? "请等待当前材料上传完成" : (state.ocrService.available ? "" : "点击后会自动启动本机扫描服务"));
  }
...
async function startOcrServiceFromPage() {
  ...
  if (message) message.textContent = "正在启动 Docling Serve 与 RapidOCR，首次加载可能需要 10 至 30 秒";

  const response = await fetch(`${API_BASE}/ocr/start`, { method: "POST" });
  const data = await response.json();
  if (response.status === 401) {
    await logout({ expired: true });
...
async function startDocumentScan(application) {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE}/cases/${encodeURIComponent(application.id)}/scan`, { method: "POST" });
    const data = await response.json();
...
function pollScanStatus(application) {
  ...
  const poll = async () => {
    ...
    polling = true;
    try {
      const response = await fetch(`${API_BASE}/cases/${encodeURIComponent(application.id)}/scan-status`);
      const data = await response.json();
      if (!response.ok) {
...
async function generateClientIntakeLink(application, container) {
  ...
  try {
    const response = await fetch(`${API_BASE}/cases/${encodeURIComponent(application.id)}/intake-link`, { method: "POST" });
    const data = await response.json();
...
function renderPrefill(container) {
  ...
  container.innerHTML = `
    ${renderAppHeader(application, "Computer Use 逐页填写", "DocFlow 准备当前客户字段计划。你人工完成验证码并进入正式表格后，再把可见页面交给 Codex Computer Use 稳健填写。")} 
    ...
    <section class="screen-agent-runtime-alert ${state.apiAvailable ? "ready" : "blocked"}" role="status">
      ...
      <span>${needsFreshHandoff ? "为避免把一次性令牌写入数据库，页面刷新后需要撤销旧任务并生成新的本机交接。客户字段仍保留在档案中。" : "DocFlow 只准备短时字段任务并打开 CEAC；实际点击、输入、下拉选择与页面复读由 Codex Desktop 的 Computer Use 完成。"}</span>
    </section>
    ${preflightIssues.length ? `
...
function computerUsePrompt(handoff, workflowType = "ds160") {
  ...
  return [
    `请使用 Computer Use 执行 DocFlow 当前 ${workflowType === "appointment" ? "预约资料" : "DS-160"} 任务。`,
...
async function markComputerUseWaiting(handoff) {
  const response = await fetch(`${handoff.taskUrl}/status`, {
    method: "POST",
    headers: {
...
async function startComputerUseAgent(application) {
  ...
  if (!state.apiAvailable || !API_BASE) {
    updateCodexAgentMessage("请先通过本地服务器打开 DocFlow。", "error");
    return;
  }
  if (button) {
    button.disabled = true;
  ...
  try {
    ...
    if (previousAgent?.jobId && !previousClosed) {
      await fetch(`${API_BASE}/cases/${encodeURIComponent(application.id)}/codex-agent/${encodeURIComponent(previousAgent.jobId)}`, {
        method: "DELETE"
      }).catch(() => {});
      clearCodexAgentPolling();
    }
    const response = await fetch(`${API_BASE}/cases/${encodeURIComponent(application.id)}/codex-agent/prepare`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    if (preparedJobId) {
      fetch(`${API_BASE}/cases/${encodeURIComponent(application.id)}/codex-agent/${encodeURIComponent(preparedJobId)}`, {
        method: "DELETE"
      }).catch(() => {});
    }
  }
...
async function refreshCodexAgent(application, options = {}) {
  ...
  try {
    const response = await fetch(`${API_BASE}/cases/${encodeURIComponent(application.id)}/codex-agent/${encodeURIComponent(agent.jobId)}`);
    const data = await response.json();
...
async function launchDesktopScreenAgent(application, plan) {
  ...
  try {
    const response = await fetch(`${API_BASE}/cases/${encodeURIComponent(application.id)}/screen-agent/run`, {
      method: "POST"
    });
    const data = await response.json();
...
async function refreshDesktopScreenAgent(application, plan) {
  ...
  try {
    const response = await fetch(`${API_BASE}/cases/${encodeURIComponent(application.id)}/screen-agent/${encodeURIComponent(desktop.jobId)}`);
    const data = await response.json();
...
async function stopDesktopScreenAgent(application, plan) {
  ...
  if (!desktop?.jobId || !state.apiAvailable || !API_BASE) return;
  try {
    const response = await fetch(`${API_BASE}/cases/${encodeURIComponent(application.id)}/screen-agent/${encodeURIComponent(desktop.jobId)}/stop`, {
      method: "POST"
    });
    const data = await response.json();
...
function renderAppointmentAccount(container) {
  ...
  container.innerHTML = `
    ...
    <section class="appointment-account-layout">
      <div class="appointment-account-process">
        ...
        <ol class="appointment-account-steps">
          ...
          <li><span>3</span><div><strong>完成账户安全设置</strong><p>密码、邮箱验证码和 3 个密保问题由顾问在官网完成，DocFlow 只保存完成状态。</p></div></li>
          ...
          <li><span>5</span><div><strong>开始新申请</strong><p>进入 Applicant Details 后，DocFlow 再带入客户资料和预约选项。</p></div></li>
        </ol>
    ...
    <section class="appointment-account-safety" role="note">
      ...
      <span>这些内容属于预约账户凭据。DocFlow 只记录“顾问已完成”，不读取密码、验证码或密保答案，也不会把它们写入客户档案或数据库。</span>
    </section>
...
async function startAppointmentAgent(application) {
  ...
  if (!state.apiAvailable || !API_BASE) {
    updateAppointmentAgentMessage("请通过本地完整版本打开 DocFlow。", "error");
    return;
  }
  ...
  try {
    ...
    if (previous?.jobId && !previousClosed) {
      await fetch(`${API_BASE}/cases/${encodeURIComponent(application.id)}/codex-agent/${encodeURIComponent(previous.jobId)}`, { method: "DELETE" }).catch(() => {});
    }
    const response = await fetch(`${API_BASE}/cases/${encodeURIComponent(application.id)}/appointment-agent/prepare`, { method: "POST" });
    const data = await response.json();
    ...
    if (preparedJobId) {
      fetch(`${API_BASE}/cases/${encodeURIComponent(application.id)}/codex-agent/${encodeURIComponent(preparedJobId)}`, { method: "DELETE" }).catch(() => {});
    }
  }
}
...
async function revokeAppointmentAgent(application) {
  ...
  if (!agent?.jobId || !state.apiAvailable || !API_BASE) return;
  try {
    const response = await fetch(`${API_BASE}/cases/${encodeURIComponent(application.id)}/codex-agent/${encodeURIComponent(agent.jobId)}`, { method: "DELETE" });
    const data = await response.json();
...
function renderPdfReportPages(application, sections) {
  ...
  const newPage = () => {
    ...
    context.fillText("DocFlow DS-160", left, 58);
  ...
  app.innerHTML = `
    <main class="public-intake-shell">
      <header class="public-intake-header">
        <div class="brand-line"><span class="brand-dot"></span><span lang="en">DocFlow DS-160</span></div>
