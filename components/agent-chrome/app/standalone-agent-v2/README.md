...
这是与现有 `standalone-agent` 并存的第二套执行层。共享浏览器驱动只增加可覆盖
的下拉框输入钩子；V1 默认行为保持不变，真实输入实现只在 V2 启用。
...
7. **全部下拉框使用真实鼠标键盘**
   V2 仍用只读 DOM 语义精确定位和核验选项，但普通下拉、日期、停留时长、
   Travel 两级框及职业联动框的实际改值使用平台系统级输入：macOS 使用
   `CGEvent`，Linux 使用 X11/XTEST（`xdotool`）。页面收到的是浏览器可信
   事件；系统输入不可用时会停止，不会回退到 JavaScript 改 `value`。
...
V2 启动前还会检查系统级输入通道：macOS 需要“辅助功能”权限，
Linux 需要可用的 Xvfb `DISPLAY` 和 XTEST。健康接口中的 `selectInputReady`
必须为 `true`。
