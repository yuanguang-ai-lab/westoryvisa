...
from scripts.run_local import (  # noqa: E402
    ManagedLocalService,
    available_port,
)
...
def main():
    ...
    configured_agent_python = os.environ.get(
        "DOCFLOW_AGENT_PYTHON", ""
    ).strip()
    if configured_agent_python:
        # Keep the venv launcher path intact. Resolving its ``python`` symlink
        # selects the base interpreter and silently drops the venv packages.
        agent_python = Path(configured_agent_python).expanduser()
        if not agent_python.is_absolute():
            agent_python = (PROJECT_ROOT / agent_python).absolute()
        if not agent_python.is_file():
            raise RuntimeError(
                "DOCFLOW_AGENT_PYTHON 指向的 Python 不存在："
                f"{agent_python}"
            )
    else:
