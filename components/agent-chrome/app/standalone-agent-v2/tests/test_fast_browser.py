...
from visa_agent_v2.browser import FastVisiblePlaywrightBrowser
...
class DelayedContactUnknownLocator(ContactUnknownLocator):
    def click(self, timeout=None):
        self.clicks += 1
        self.page.checked = not self.page.checked
        self.page.pending_names_unavailable = self.page.checked


class DelayedContactUnknownPage(ContactUnknownPage):
    def __init__(self, checked=True, names_unavailable=False):
        super().__init__(checked, names_unavailable)
        self.pending_names_unavailable = names_unavailable
        self.wait_calls = 0
        self.checkbox = DelayedContactUnknownLocator(self)

    def wait_for_timeout(self, _milliseconds):
        self.wait_calls += 1
        if self.wait_calls >= 1:
            self.names_unavailable = self.pending_names_unavailable
...
class FamilyIdentityPage:
    ...
    def evaluate(self, script, _argument=None):
        ...
        # fails, but the exact US_OTHER_RELATIVE_IND identity is unique.
        return "otherrelativeind" in script
...
class UncheckedChoiceLocator(CheckedChoiceLocator):
    @staticmethod
    def is_checked(timeout=None):
        return False
...
class FastBrowserTests(unittest.TestCase):
    ...
    def test_us_contact_required_block_cannot_be_classified_absent(self):
        ...
        browser._us_contact_controller_control = lambda _field_id: object()
        browser._us_contact_semantic_control = (
            lambda _field_id, kind: ContactControlLocator(kind)
        )
        ...
        browser._us_contact_controller_control = lambda _field_id: None
        browser._us_contact_semantic_control = lambda *_args, **_kwargs: None
    ...
    def test_us_contact_controllers_cannot_be_classified_absent(self):
        browser = FastVisiblePlaywrightBrowser(
            ProviderConfig(provider="playwright", model="chromium-headless")
        )
        browser._page = ContactPage()
        labels = {
            "ceac.us_contact.us_contact.person.does_not_know": (
                "Do Not Know contact person [control=yes_no]",
            ),
            "ceac.us_contact.us_contact.organization": (
    ...
    def test_us_contact_unknown_waits_for_delayed_ceac_controller_state(self):
        browser = FastVisiblePlaywrightBrowser(
            ProviderConfig(provider="playwright", model="chromium-headless")
        )
        page = DelayedContactUnknownPage(
            checked=True,
            names_unavailable=False,
        )
        browser._page = page
    ...
    def test_choice_hint_fallback_repairs_persisted_localized_work3_fields(self):
        fields = [
            "ceac.work_education3.additional.specialized_skills.90b91da7a31d",
            "ceac.work_education3.additional.military_service.54e3856649b2",
            "ceac.work_education3.additional.paramilitary.22690a682a10",
        ]

        hints = FastVisiblePlaywrightBrowser._complete_choice_control_hints(
            fields,
            {},
        )

        self.assertIn("SPECIALIZED_SKILLS_IND", hints[fields[0]])
        self.assertIn("MILITARY_SERVICE_IND", hints[fields[1]])
        self.assertIn("INSURGENT_ORG_IND", hints[fields[2]])

    def test_choice_hint_fallback_repairs_persisted_security2_fields(self):
        fields = [
            "ceac.security_background2.security.trafficking_participation.111111111111",
            "ceac.security_background2.security.trafficking_assistance.222222222222",
            "ceac.security_background2.security.trafficking_benefit.333333333333",
        ]
        old_hints = {
            fields[0]: ["TRAFFICKING_OFFENSE"],
            fields[1]: ["TRAFFICKING_ASSIST"],
            fields[2]: ["TRAFFICKING_FAMILY"],
        }
        ...
        self.assertIn("HUMAN_TRAFFICKING", hints[fields[0]])
        self.assertIn("ASSISTED_SEVERE_TRAFFICKING", hints[fields[1]])
        self.assertIn("HUMAN_TRAFFICKING_RELATED", hints[fields[2]])

    def test_choice_hint_fallback_repairs_persisted_security3_fields(self):
        fields = [
            "ceac.security_background3.security.torture.111111111111",
            "ceac.security_background3.security."
            "forced_abortion_sterilization.222222222222",
            "ceac.security_background3.security."
            "forced_organ_transplant.333333333333",
        ]
        ...
        self.assertIn("TORTURE", hints[fields[0]])
        self.assertIn("POPULATION_CONTROL", hints[fields[1]])
        self.assertIn("BODILY_TISSUE", hints[fields[2]])

    def test_security_choice_presence_never_discards_unanswered_radios(self):
        fields = [
            "ceac.security_background3.security.torture.111111111111",
            "ceac.security_background3.security."
            "forced_abortion_sterilization.222222222222",
        ...
        labels = {
            field_id: (
                "Approved security question [control=yes_no; "
        ...
        result = (
            FastVisiblePlaywrightBrowser._classify_security_choice_presence(
                {"present": [], "absent": fields, "unresolved": []},
                fields,
                labels,
            )
        )
        ...
        self.assertEqual(result["unresolved"], fields)

    def test_security_part4_variant_fields_can_be_absent(self):
        visible_fields = [
            "ceac.security_background4.immigration."
            "fraud_misrepresentation.91f3a76ef74c",
            "ceac.security_background4.immigration."
            "removal.ffac4bb89fe1",
        ]
        variant_fields = [
            "ceac.security_background4.immigration."
            "status_violation.8869082064f0",
            "ceac.security_background4.immigration."
            "assisted_illegal_entry.3a8828cb65b4",
        ]
        fields = [*visible_fields, *variant_fields]
        labels = {
            field_id: (
                "Approved security question [control=yes_no; "
                "human-approved value=no]",
        ...
        result = (
            FastVisiblePlaywrightBrowser._classify_security_choice_presence(
                {
                    "present": visible_fields,
                    "absent": variant_fields,
                    "unresolved": [],
                },
                fields,
                labels,
        ...
        self.assertEqual(result["present"], visible_fields)
        self.assertEqual(result["absent"], variant_fields)
        self.assertEqual(result["unresolved"], [])

    def test_late_family_probe_is_limited_to_lone_missing_dependent(self):
        ...
        browser._page = RelativesPage()
        browser._family_immediate_choice_answered = lambda: True
        browser._family_choice_group = lambda _field_id, _terms: None

        self.assertEqual(
            browser.late_required_validation_probe_fields([field_id]),
            [field_id],
        )
        self.assertEqual(
            browser.late_required_validation_probe_fields(
    ...
    def test_work_education1_exact_value_proof_uses_prompt_scoped_control(self):
        browser = FastVisiblePlaywrightBrowser(
            ProviderConfig(provider="playwright", model="chromium-headless")
        )
        ...
        browser._page = WorkPage()
        browser._travel_semantic_control = (
            lambda _terms, kind, **_kwargs: WorkValueLocator({
                "tag": "select" if kind == "select_text" else "input",
                "type": "" if kind == "select_text" else "text",
                "value": "B" if kind == "select_text" else "SS HOTEL",
                "checked": False,
                "selectedText": "BUSINESS" if kind == "select_text" else "",
            })
        )

        self.assertTrue(browser.work_education1_exact_value_matches(
            "ceac.work_education1.work.primary_occupation",
            "BUSINESS",
        ))
        self.assertTrue(browser.work_education1_exact_value_matches(
        ...
        self.assertFalse(browser.work_education1_exact_value_matches(
            "ceac.work_education1.work.organization",
            "DIFFERENT HOTEL",
        ))

    def test_work_missing_branch_replay_is_armed_on_first_plan(self):
        browser = FastVisiblePlaywrightBrowser(
            ProviderConfig(provider="playwright", model="chromium-headless")
        )
        ...
        browser._page = WorkPage()
        browser._plan_work_semantic_fallback = (
            lambda _field_ids, _labels: (
                [ComputerAction(
                    kind=ActionKind.SELECT,
                    field_id=primary_id,
                    target_hint=primary_id,
                )],
                [organization_id],
            )
        )

        actions, unresolved = browser._plan_work_page_once(
        ...
        self.assertEqual([item.field_id for item in actions], [primary_id])
        self.assertIn(primary_id, browser._v2_forced_postback_field_ids)
        self.assertFalse(browser._v2_work_reopen_attempted)
        self.assertIn("first-pass missing-branch", actions[0].reason)

        # If the fresh placeholder selection was accepted without rendering
        # its panel, the very next deterministic plan must still be allowed to
        # issue the one guarded placeholder -> reviewed replay.
        browser._v2_forced_postback_field_ids.discard(primary_id)
        replay = browser._plan_missing_work_branch_replay(
            [organization_id],
            {primary_id: ("Primary Occupation",)},
        )
        self.assertEqual([item.field_id for item in replay], [primary_id])

    def test_work_exact_proof_prefers_action_owned_marker(self):
        browser = FastVisiblePlaywrightBrowser(
            ProviderConfig(provider="playwright", model="chromium-headless")
        )
        field_id = (
            "ceac.work_education1.work.present.address.0.region.35b73dae3a68"
        )
        browser._is_work_education1_page = lambda: True
        browser._field_selectors[field_id] = (
            f'[data-docflow-field="{field_id}"]'
        )
        browser._live_control_value = (
            lambda requested, selector, timeout: "GUANGDONG PROVINCE"
        )
        browser._travel_semantic_control = lambda *_args, **_kwargs: (
            self.fail("broad prompt binder must not run after exact proof")
        )
    ...
    def test_work2_postback_rebind_waits_for_replacement_document(self):
        browser = FastVisiblePlaywrightBrowser(
            ProviderConfig(provider="playwright", model="chromium-headless")
        )
        field_id = "ceac.work_education2.work.previously_employed"
        action = ComputerAction(
            kind=ActionKind.SELECT,
            field_id=field_id,
            target_hint=field_id,
        ...
        initial = UncheckedChoiceLocator()
        rebound = CheckedChoiceLocator()
        browser._page = FakePage()
        browser._action_locator = lambda _action: initial
        browser._travel_specific_plans_choice_control = (
            lambda group, _value: initial if group is initial else rebound
        )
        browser._prompt_scoped_choice_group = (
            lambda _terms: (
                None if browser._page.wait_calls == 0 else "rebound"
            )
        )
        browser._work_education2_choice_terms = lambda _field_id: ("work",)
        browser._mark_field = lambda *_args: None
        browser._begin_action_dom_watch = lambda: None
        browser._ensure_travel_control_postback = (
            lambda *_args, **_kwargs: True
        )
        browser._select_best_page = lambda: None
        browser._configure_timeout_target = lambda _page: None
        browser._prune_detached_field_bindings = lambda: []

        browser._execute_work_education2_choice(action)

        self.assertIn(action.id, browser._acknowledged)
        self.assertGreaterEqual(browser._page.wait_calls, 1)
    ...
    def test_delayed_family_followup_remains_pending_after_controller(self):
        browser = FastVisiblePlaywrightBrowser(
            ProviderConfig(provider="playwright", model="chromium-headless")
        )
        browser._page = FakePage()
        ...
        self.assertEqual(presence["absent"], [])
        self.assertEqual(presence["unresolved"], [field_id])
    ...
    def test_travel_secondary_revalidation_accepts_connector_equivalence(self):
        ...
        self.assertTrue(browser.travel_purpose_matches_approved(
            "ceac.travel.travel.purpose.secondary.45f2572d8486",
            "BUSINESS & TOURISM (TEMPORARY VISITOR) (B1/B2)",
        ))

    def test_hashed_travel_secondary_is_staged_before_payer(self):
        browser = FastVisiblePlaywrightBrowser(
            ProviderConfig(provider="playwright", model="chromium-headless")
        )
        browser._is_passport_page = lambda: False
        browser._is_relatives_page = lambda: False
        browser._is_work_education1_page = lambda: False
        browser._is_work_education2_page = lambda: False
        browser._is_us_contact_page = lambda: False
        browser._is_address_phone_page = lambda: False
        browser._is_travel_page = lambda: True
        secondary = (
            "ceac.travel.travel.purpose.secondary.45f2572d8486"
        )
        payer = "ceac.travel.travel.payer.f399087a9961"
        action = ComputerAction(
            kind=ActionKind.SELECT,
            field_id=secondary,
            target_hint=secondary,
            value="BUSINESS & TOURISM (TEMPORARY VISITOR) (B1/B2)",
        )

        with patch.object(
            browser,
            "_plan_travel_semantic_fallback",
            return_value=([action], []),
        ) as semantic_plan:
            actions, unresolved = browser._plan_semantic_fields_once(
                [secondary, payer],
        ...
        self.assertEqual(actions, [action])
        self.assertEqual(unresolved, [payer])
        self.assertEqual(semantic_plan.call_args.args[0], [secondary])
