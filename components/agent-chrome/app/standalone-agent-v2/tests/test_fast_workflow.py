...
class SemanticBrowser(MockBrowserDriver):
    ...
    def family_date_value_matches(self, field_id, approved_value):
        return (
            str(field_id).endswith(".family.father.dateofbirth")
            and str(approved_value) == "1976-05-30"
        )
    ...
    def work_education1_exact_value_matches(
        ...
        return (
            ".work_education1." in str(field_id)
            and str(approved_value) == "XINZHUOSHIYE"
        )
...
class LiveFamilyValidationProbeBrowser(SemanticBrowser):
    def __init__(self, field_id):
        ...
        self.title = "Family Information: Relatives"
        self.visible_text = self.title
        self.revealed = False
        self.validation_errors = []

    def plan_fields(self, field_ids, labels, hints):
    ...
    def late_required_validation_probe_fields(
        ...
        return list(field_ids) if not self.revealed else []

    def execute(self, action):
        if action.kind == ActionKind.CLICK and not self.revealed:
            self.executed.append(action)
            self.acknowledged_action_ids.append(action.id)
            self.revealed = True
            self.validation_errors = [
                "Please correct all areas in error as indicated below.",
                "Other Relatives in the United States has not been completed.",
            ]
            return
        if action.field_id == self.field_id:
            self.validation_errors = []
        return super().execute(action)
...
class FastWorkflowTests(unittest.TestCase):
    ...
    def test_work_education3_inconclusive_readback_keeps_next_locked(self):
        field_id = "ceac.work_education3.additional.countries_visited"
        browser = SemanticBrowser()
        job = AgentJob(
            fields=[confirmed_field(field_id, "no")],
            start_url=(
        ...
        stale, inconclusive = agent._stale_completed_page_fields(
            ...
            [field_id],
            {field_id: (
                "Have you traveled to any countries within the last five "
                "years? [control=yes_no]",
            )},
            {},
            None,
        )
    ...
    def test_late_family_validation_maps_live_ceac_short_wording(self):
        field_id = (
            "ceac.relatives.family.other_relatives_us.118edc73f3be"
        )
        errors = [
            "Please correct all areas in error as indicated below.",
            "Other Relatives in the United States has not been completed.",
    ...
    def test_live_family_probe_repairs_new_field_without_restart(self):
        field_id = (
            "ceac.relatives.family.other_relatives_us.118edc73f3be"
        )
        browser = LiveFamilyValidationProbeBrowser(field_id)
        job = AgentJob(
            fields=[confirmed_field(field_id, "no")],
            start_url=browser.url,
            required_field_ids=[field_id],
        ...
        self.assertNotEqual(result.state, JobState.WAITING_HUMAN)
        self.assertEqual(browser.next_clicks, 1)
        self.assertTrue(any(
            action.field_id == field_id for action in browser.executed
        ))
        self.assertTrue(any(
    ...
    def test_work_repair_ignores_only_stale_submit_error_with_exact_proof(self):
    ...
    def test_work_repair_keeps_real_value_mismatch_blocked(self):
        field_id = "ceac.work_education1.work.organization"
        agent = FastComputerUseAgent(
            ModelMustNotRun(),
            SemanticBrowser(),
    ...
    def test_repaired_family_date_ignores_only_stale_submit_error(self):
        field_id = "ceac.relatives.family.father.dateofbirth"
        agent = FastComputerUseAgent(
            ModelMustNotRun(),
            SemanticBrowser(),
            execution_mode="hybrid",
