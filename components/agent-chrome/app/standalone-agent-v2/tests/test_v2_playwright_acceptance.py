...
class RecordingFastBrowser(FastVisiblePlaywrightBrowser):
    def __init__(self):
        # V2 dropdowns are intentionally system-level input and therefore
        # require a visible browser even in acceptance coverage.
...
class V2PlaywrightAcceptanceTests(unittest.TestCase):
    def test_live_travel_select_resolves_option_for_native_input(self):
        class RecordingNativeBrowser(FastVisiblePlaywrightBrowser):
            def __init__(self):
                super().__init__(browser_provider())
                self.native_options = []

            def _activate_select_option(self, _locator, selected):
                self.native_options.append(selected)
                return True

        browser = RecordingNativeBrowser()
        ...
        self.assertEqual(len(browser.native_options), 1)
        self.assertEqual(browser.native_options[0]["value"], "B")

    def test_headed_chromium_travel_commit_emits_change(self):
        """The visible controlled postback must create the child select."""
        ...
        try:
            ...
            browser._page.set_content("""
                <title>DocFlow Native Select Fixture</title>
                <style>
    ...
    def test_travel_primary_waits_for_async_postback_before_rebinding(self):
        ...
        try:
            ...
            # Production provider manifests append a stable instance hash.
            # The two-level Travel barrier must not depend on fixture-only,
            # unhashed IDs or it will let Payer race the missing Specify box.
            primary = "ceac.travel.travel.purpose.primary.0d21431122f9"
            secondary = "ceac.travel.travel.purpose.secondary.45f2572d8486"
            labels = {
                primary: (
    ...
    def test_family_parent_panels_are_present_filled_and_read_back(self):
        ...
        try:
            ...
            self.assertTrue(browser.family_date_value_matches(
                "ceac.relatives.family.father.dateofbirth",
                "1976-05-30",
            ))
            self.assertTrue(browser.family_date_value_matches(
            ...
            self.assertFalse(browser.family_date_value_matches(
                "ceac.relatives.family.father.dateofbirth",
                "1976-05-29",
            ))
    ...
    def test_security2_exact_control_suffix_beats_shared_trafficking_hint(self):
        ...
        try:
                      ...
                      <div class="question">
                        ...
                        <span class="answers">
                          <input id="assist-y" type="radio"
                                 name="ctl00$rblAssistedSevereTrafficking"
                                 value="Y">
                      ...
                      <div class="question">
                        ...
                        <span class="answers">
                          <input id="family-y" type="radio"
                                 name="ctl00$rblHumanTraffickingRelated"
                                 value="Y">
            ...
            field_ids = [
                ...
                "trafficking_participation.111111111111",
                "ceac.security_background2.security."
                "trafficking_assistance.222222222222",
                "ceac.security_background2.security."
                "trafficking_benefit.333333333333",
            ]
            labels = {
                field_ids[0]: (
                ...
                field_ids[1]: (
                    "Have you ever knowingly aided, abetted, assisted or "
                    "colluded in severe human trafficking? "
                    "[control=yes_no; human-approved value=no]",
                ),
                field_ids[2]: (
                    "Have you knowingly benefited from trafficking activity? "
                    "[control=yes_no; human-approved value=no]",
                ),
            }
            hints = {
                field_ids[0]: ("TRAFFICKING_OFFENSE",),
                field_ids[1]: ("TRAFFICKING_ASSIST",),
                field_ids[2]: ("TRAFFICKING_FAMILY",),
            }

            actions, unresolved = browser.plan_choice_fields(
                field_ids, labels, hints,
            )
            self.assertEqual(unresolved, [])
            self.assertEqual(len(actions), 3)
            expected_names = [
                "ctl00$rblHumanTrafficking",
                "ctl00$rblAssistedSevereTrafficking",
                "ctl00$rblHumanTraffickingRelated",
            ]
            for field_id, expected_name in zip(field_ids, expected_names):
                selector = browser._field_selectors[field_id]
                self.assertEqual(
                    browser._page.locator(selector).get_attribute("name"),
                    expected_name,
            ...
            self.assertTrue(browser._page.locator("#assist-n").is_checked())
            self.assertTrue(browser._page.locator("#family-n").is_checked())
        finally:
            browser.close()

    def test_work3_localized_persisted_fields_bind_all_required_radios(self):
        browser = RecordingFastBrowser()
        page_url = (
            "https://ceac.state.gov/GenNIV/General/complete/"
            "complete_workeducation3.aspx?node=WorkEducation3"
        ...
        try:
            ...
            browser._page.route(
                ...
                lambda route: route.fulfill(
                    ...
                    <form>
                      <div class="question">
                        <span class="prompt">Do you have any specialized
                          skills or training, such as firearms, explosives,
                          nuclear, biological, or chemical experience?</span>
                        <span class="answers">
                          <input id="skills-y" type="radio"
                            name="ctl00$rblSPECIALIZED_SKILLS_IND" value="Y">
                          <input id="skills-n" type="radio"
                            name="ctl00$rblSPECIALIZED_SKILLS_IND" value="N">
                        </span>
                      </div>
                      <div class="question">
                        <span class="prompt">Have you ever served in the
                          military?</span>
                        <span class="answers">
                          <input id="military-y" type="radio"
                            name="ctl00$rblMILITARY_SERVICE_IND" value="Y">
                          <input id="military-n" type="radio"
                            name="ctl00$rblMILITARY_SERVICE_IND" value="N">
                        </span>
                      </div>
                      <div class="question">
                        <span class="prompt">Have you ever served in, been a
                          member of, or been involved with a paramilitary or
                          insurgent organization?</span>
                        <span class="answers">
                          <input id="insurgent-y" type="radio"
                            name="ctl00$rblINSURGENT_ORG_IND" value="Y">
                          <input id="insurgent-n" type="radio"
                            name="ctl00$rblINSURGENT_ORG_IND" value="N">
                        </span>
                      </div>
                    </form>
                    """,
            ...
            labels = {
                field_id: (
                    f"已审核中文问题 {index} [control=yes_no; "
                    "human-approved value=no]",
                )
            ...
            self.assertEqual(set(presence["present"]), set(field_ids))
            self.assertEqual(presence["absent"], [])
            ...
            self.assertTrue(browser._page.locator("#skills-n").is_checked())
            self.assertTrue(browser._page.locator("#military-n").is_checked())
            self.assertTrue(browser._page.locator("#insurgent-n").is_checked())
            self.assertEqual(
                browser.unanswered_visible_choice_fields(
                    field_ids,
                    labels,
