...
from .chrome_process import ChromeProcessManager, ChromeProcessError
from .field_ids import semantic_field_id, semantic_field_id_endswith
from .native_input import NativeInputUnavailable, create_native_input
...
class FastVisiblePlaywrightBrowser(PlaywrightBrowserDriver):
    ...
    def __init__(self, config):
        ...
        try:
            self._native_input = create_native_input()
        except NativeInputUnavailable as error:
            self._native_input = None
            self._native_input_error = error
        else:
            self._native_input_error = None
        if (
            getattr(self._native_input, "name", "")
            == "linux-x11-xtest"
        ):
            # The Linux virtual desktop includes roughly 8 px of horizontal
            # frame and 131 px of Chromium UI around Playwright's viewport.
            # Keep the complete window inside the 1280x800 Xvfb framebuffer so
            # both streaming and any OS-level input stay on-screen.
            self.width = int(os.environ.get(
                "DOCFLOW_BROWSER_VIEWPORT_WIDTH", "1272"
            ))
            self.height = int(os.environ.get(
                "DOCFLOW_BROWSER_VIEWPORT_HEIGHT", "669"
            ))
    ...
    def start(self, url):
        """Require the system input channel before opening a V2 job."""
        if self._native_input is None:
            raise self._native_input_error or NativeInputUnavailable(
                "V2 系统级鼠标键盘通道不可用。"
            )
        self._native_input.require_authorized()
    ...
    def _apply_structured_field_value(self, locator, action):
        ...
        if (
            ...
            and semantic_field_id_endswith(field_id, (
        ...
        if (
            self._is_travel_page()
            and semantic_field_id_endswith(
                field_id, "travel.stayduration"
            )
        ):
    ...
    def execute(self, action):
        ...
        if (
            action.kind == ActionKind.SELECT
            and semantic_field_id_endswith(
                field_id, ".travel.specific_plans"
            )
        ):
        ...
        if (
            ...
            and semantic_field_id_endswith(
                field_id, ".us_contact.person.does_not_know"
            )
        ):
        ...
        if (
            action.kind == ActionKind.SELECT
            and semantic_field_id_endswith(field_id, ".travel.payer")
        ):
        ...
        if (
            ...
            self._v2_forced_postback_field_ids.discard(field_id)
            # Planning a first-pass repair is not the same as attempting it.
            # A fresh CEAC page normally starts at the placeholder, so this
            # first action must select the reviewed occupation once.  Keep the
            # bounded repair available for the immediately following plan if
            # CEAC accepts that value but still withholds the employer panel.
            self._v2_work_reopen_attempted = False
            return super().execute(action)
        ...
        # From this point the browser has proved that the reviewed controller
        # is already selected and the guarded placeholder -> reviewed replay
        # will actually be attempted.  Only now consume the one-shot budget.
    ...
    def _wait_for_us_contact_state(self, predicate):
        """Wait for CEAC's checkbox controller, not a fixed paint delay.

        U.S. Contact uses client-side handlers to synchronize the visible
        checkbox, disabled dependent inputs, and hidden ASP.NET value.  On the
        Linux/Xvfb production browser that synchronization can complete after
        the trusted click returns.  A fixed 180 ms sleep therefore made the
        first attempt fail and a later resume succeed by accident.
        """
        for delay_ms in (0, 60, 100, 160, 250, 400, 650, 900):
            if delay_ms:
                try:
    ...
    def _wait_for_us_contact_person_toggle(self, desired):
        return self._wait_for_us_contact_state(
            lambda: self._us_contact_person_toggle_consistent(desired)
        )
    ...
    def _execute_us_contact_person_unknown(self, action):
        ...
        if not self._us_contact_person_toggle_consistent(desired):
            ...
            if current == bool(desired):
                ...
                if not self._wait_for_us_contact_person_toggle(not desired):
                    self.invalidate_field_binding(str(action.field_id or ""))
                    return
                locator = self._us_contact_person_unknown_control()
                if locator is None:
                    ...
                    return
            self._trusted_us_contact_checkbox_click(locator)
        if not self._wait_for_us_contact_person_toggle(desired):
            self.invalidate_field_binding(str(action.field_id or ""))
    ...
    def _execute_us_contact_email_dna(self, action):
        ...
        if not self._us_contact_email_dna_consistent(state):
            if bool(state.get("checked")):
                self._trusted_us_contact_checkbox_click(locator)
                if not self._wait_for_us_contact_state(
                    lambda: not bool(
                        self._us_contact_email_dna_state().get("checked")
                    )
                ):
                    self.invalidate_field_binding(field_id)
                    return
            ...
            self._trusted_us_contact_checkbox_click(locator)
        if not self._wait_for_us_contact_state(
            lambda: self._us_contact_email_dna_consistent(
    ...
    def us_contact_relationship_matches_approved(self, field_id, approved):
        ...
        if (
            ...
            or not semantic_field_id_endswith(
                field_id, ".us_contact.relationship"
            )
        ):
            return False
    ...
    def action_postcondition(self, action):
        ...
        if semantic_field_id_endswith(
            ...
            return True, ""
        if semantic_field_id_endswith(
            field_id, ".work.education_secondary_or_above"
        ):
        ...
        if not semantic_field_id_endswith(
            field_id, ".travel.purpose.primary"
        ):
            return True, ""
        if not self._is_travel_page():
            return (
    ...
    def action_postcondition_requires_hard_boundary(self, action):
        ...
        field_id = str(getattr(action, "field_id", "") or "")
        if (
            getattr(action, "kind", None) == ActionKind.SELECT
            and getattr(self._native_input, "name", "")
            == "linux-x11-xtest"
        ):
            # A Linux select that failed exact DOM verification or its trusted
            # event audit must never be auto-resumed into a refill loop.
            return True
        if (
            semantic_field_id_endswith(
                field_id, ".travel.stayduration"
    ...
    def _execute_travel_purpose_branch_reset(self, action):
        ...
        if secondary is None:
            ...
            self._verified_field_values.pop(field_id, None)
            self._v2_forced_travel_purpose_field_ids.discard(field_id)
            self._restore_fresh_travel_page("purpose-secondary-missing")
            return
    ...
    def _post_travel_purpose_value(
        ...
        *,
        ...
        if isinstance(audit, dict) and (
            int(audit.get("change") or 0) < 1
            or audit.get("changeTrusted") is not True
        ):
            raise RuntimeError(
                "Native select event audit did not prove CEAC postback "
                "wiring: "
        ...
        try:
            ...
            # A same-value manual reselect is a browser no-op. Restore the
            # fresh server-rendered page before pausing so a human can choose
            # the reviewed option again without first having to reload.
            try:
                self._page.reload(
                    wait_until="domcontentloaded",
                    timeout=self.NAVIGATION_TIMEOUT_MS,
                ...
                self._last_control_postback_diagnostic.update({
                    "safeReloadAfterUnknownPostback": False,
                    "safeReloadErrorType": type(reload_error).__name__,
                })
            raise
        try:
    ...
    def _execute_work_education2_choice(self, action):
        ...
        # A full WebForms form POST can finish at the network layer a few
        # hundred milliseconds before Playwright can inspect the replacement
        # document.  Previously that short inspection gap invalidated the
        # binding and forced a human restart even though CEAC had received the
        # answer.  Reacquire the exact radio for a bounded settle window; this
        # is read-only and never issues a second click or form POST.
        rebound = None
        for delay_ms in (0, 120, 180, 250, 350, 500):
            if delay_ms:
                try:
                    self._page.wait_for_timeout(delay_ms)
                except Exception:
                    # The old document can disappear between the POST receipt
                    # and this wait.  Continue so _select_best_page() can bind
                    # the replacement document instead of treating the normal
                    # detach as a failed answer.
                    pass
            try:
                self._select_best_page()
                self._configure_timeout_target(self._page)
                ...
                if candidate is not None and candidate.is_checked(
                    timeout=1000,
                ):
                    rebound = candidate
                    break
    ...
    def _execute_travel_stay_duration(self, action):
        ...
        if not posted or not self._wait_for_travel_us_address():
            ...
            self._verified_field_values.pop(field_id, None)
            self._restore_fresh_travel_page(
                "stay-duration-address-branch-missing"
            )
            return
        self._v2_forced_refresh_receipt_field_ids.add(field_id)
    ...
    def _restore_fresh_travel_page(self, reason):
        """Undo a local-only selection so a same-value manual retry works."""
        diagnostic = self._last_control_postback_diagnostic
        try:
            self._page.reload(
                wait_until="domcontentloaded",
                timeout=self.NAVIGATION_TIMEOUT_MS,
    ...
    def _activate_select_option(self, locator, selected):
        """Choose a native HTML option with a mouse click and real keys.
        ...
        if int(snapshot.get("selectedIndex", -1)) == target_index:
            self._last_native_select_audit = {"alreadySelected": True}
            return True
        ...
        try:
            ...
            if getattr(
                self._native_input,
                "focus_select_before_input",
                False,
            ):
                # Focusing does not change the form value.  The subsequent
                # XTEST keys remain the only state-changing input and emit
                # Chromium-trusted input/change events.
            ...
            for delay_ms in (80, 100, 150, 220, 300):
                ...
                if isinstance(current, dict) and (
                    ...
                    if getattr(
                        ...
                        audit = dict(self._last_native_select_audit or {})
                        if not (
                            int(audit.get("input") or 0) >= 1
                            and int(audit.get("change") or 0) >= 1
                            and audit.get("inputTrusted") is True
                            and audit.get("changeTrusted") is True
                        ):
                            self._last_native_select_failure = (
                                "linux-trusted-event-audit-mismatch"
                            )
                            return False
                    return True
    ...
    def native_select_diagnostics(self):
        """Expose non-sensitive proof for operational monitoring."""
        audit = dict(getattr(self, "_last_native_select_audit", None) or {})
        failure = str(getattr(self, "_last_native_select_failure", "") or "")
        return {
            "backend": str(getattr(self._native_input, "name", "") or ""),
            "audit": {
                key: audit.get(key)
                for key in (
                    "alreadySelected",
    ...
    def _screen_point_for_box(self, box):
        """Convert a Playwright viewport box to a global desktop point."""
    ...
    def _ensure_travel_control_postback(
        ...
        *,
        ...
        """Prove the native change reached CEAC, or submit its exact target.

        A real macOS menu click owns the value transition and trusted change
        event.  Production CEAC can nevertheless leave that value local when
        its inline WebForms callback does not start traffic.  First give the
        native callback a bounded chance to produce DOM/async/network proof.
        ...
        if not (
            ...
            return False
        raise RuntimeError(
            "CEAC Travel purpose changed through native input, but its "
            ...
            if self._v2_network_started > network_before:
                ...
                if network_outcome != "network-idle":
                    ...
                    raise ControlPostbackTimeout(
                        "CEAC Travel purpose postback started but did not "
                        "finish before the bounded settle timeout"
                    )
                self._last_dynamic_refresh_evidence.update({
                    "postbackDispatch": dispatch_kind,
                    "networkRequestStarted": True,
    ...
    @staticmethod
    def _complete_choice_control_hints(field_ids, control_hints=None):
        """Add conservative CEAC suffixes when reviewed metadata is sparse.

        Existing persisted jobs can predate a bridge metadata fix.  Their
        localized labels are still trustworthy values, but cannot bind to an
        English CEAC radio prompt.  Exact ASP.NET suffix hints let presence,
        planning, revalidation and the pre-Next unanswered audit share one
        durable binding contract instead of each page requiring a patch.
        """
        completed = {
            str(field_id): list(values or ())
            for field_id, values in dict(control_hints or {}).items()
        }
        aliases = (
            (
                ".additional.specialized_skills",
                ("SPECIALIZED_SKILLS_IND", "SPECIALIZED_SKILLS"),
            ),
            (
                ".additional.military_service",
                ("MILITARY_SERVICE_IND", "MILITARY_SERVICE"),
            ),
            (
                ".additional.paramilitary",
                ("INSURGENT_ORG_IND", "INSURGENT_ORG", "PARAMILITARY"),
            ),
            (
                ".security.trafficking_participation",
                ("HUMAN_TRAFFICKING",),
            ),
            (
                ".security.trafficking_assistance",
                ("ASSISTED_SEVERE_TRAFFICKING",),
            ),
            (
                ".security.trafficking_benefit",
            ...
            (
                ".security.forced_organ_transplant",
                ("ORGAN_TRANSPLANT", "BODILY_TISSUE"),
            ),
        )
        for raw_field_id in field_ids or ():
            field_id = str(raw_field_id or "")
            values = completed.setdefault(field_id, [])
            normalized = semantic_field_id(field_id).casefold()
            additions = next((
                hints for token, hints in aliases if normalized.endswith(token)
            ), ())
            if not values and not additions:
                tail = normalized.rsplit(".", 1)[-1]
                derived = re.sub(r"[^a-z0-9]+", "_", tail).strip("_")
                if len(derived) >= 4:
                    additions = (
                        f"{derived.upper()}_IND",
                        derived.upper(),
                    )
    ...
    def classify_field_presence(
        ...
        """
        control_hints = self._complete_choice_control_hints(
            field_ids,
            control_hints,
        )
        ...
        if self._is_security_background_page():
            return self._classify_security_choice_presence(
                result,
                field_ids,
                dict(field_labels or {}),
            )
    ...
    def _stale_us_contact_controllers(
        ...
        by_suffix = {
            suffix: next((
                ...
                if semantic_field_id_endswith(field_id, suffix)
            ), "")
    ...
    @staticmethod
    def _classify_security_choice_presence(
        generic_result,
        field_ids,
        field_labels,
    ):
        """Never discard an approved Part 1-5 radio from a live part page.

        The security pages contain unconditional Yes/No questions.  Several
        production control names are opaque ASP.NET ids, and a short reviewed
        label such as ``torture`` is insufficient for the generic presence
        probe.  Calling that situation absent caused an approved question to
        be removed from the page plan even though the stricter pre-Next audit
        could see its radio group.  On an exact Security/Background route,
        missing deterministic evidence is inconclusive, not proof that an
        unconditional reviewed question is inapplicable.
        """
        present = set(generic_result.get("present") or ())
        absent = set(generic_result.get("absent") or ())
        unresolved = set(generic_result.get("unresolved") or ())
        ...
        for raw_field_id in field_ids or ():
            ...
            if not any(
                ...
                continue
            if FastVisiblePlaywrightBrowser._control_kind(
                (field_labels or {}).get(field_id) or ()
            ) != "yes_no":
                continue
            scoped_ids.append(field_id)
        # Current CEAC Security/Background Part 4 renders only the fraud /
        # misrepresentation and removal/deportation questions for this form
        # variant.  Older reviewed manifests can also contain status-
        # violation and assisted-illegal-entry questions.  Those two fields
        # have no live controls or prompt text on that variant.  Preserve the
        # generic DOM classifier's authoritative absence for only these exact
        # legacy/variant fields; otherwise Gemini searches the whole page for
        # controls that do not exist and emits an unbounded sequence of
        # verified scrolls.  Every other Security Part 1-5 field retains the
        # stricter missing-is-unresolved protection below.
        part4_variant_suffixes = (
            ".immigration.status_violation",
            ".immigration.assisted_illegal_entry",
        )
        for field_id in scoped_ids:
            if field_id in absent:
                if semantic_field_id_endswith(
                    field_id, part4_variant_suffixes
                ):
                    continue
                absent.discard(field_id)
                unresolved.add(field_id)
        ...
        for field_id in scoped_ids:
            ...
            if locator is None:
                if (
                    ...
                    # CEAC inserts the Other-Relatives group through a delayed
                    # partial postback.  Linux/Xvfb can observe the controller
                    # answer before that replacement DOM is committed.  A
                    # still-missing reviewed question is therefore unresolved,
                    # never proof that the question is inapplicable.
    ...
    def _classify_us_contact_field_presence(
        ...
        scoped_ids = [
            ...
            if (
                ...
                or any(
                    semantic_field_id_endswith(field_id, suffix)
        ...
        for field_id in scoped_ids:
            ...
            if rule is None:
                locator = self._us_contact_controller_control(field_id)
                if locator is None:
                    unresolved.add(field_id)
                else:
                    present.add(field_id)
    ...
    def _us_contact_controller_control(self, field_id):
        """Return one exact U.S. Contact branch controller if rendered."""
        if semantic_field_id_endswith(
            field_id, ".us_contact.person.does_not_know"
        ):
            return self._us_contact_person_unknown_control()
        if semantic_field_id_endswith(
            field_id, ".us_contact.relationship"
        ):
            return self._us_contact_relationship_control()
        if semantic_field_id_endswith(
            field_id, ".us_contact.organization"
        ):
            try:
                controls = self._page.locator(
                    'input[id$="_tbxUS_POC_ORGANIZATION"]'
    ...
    def _is_security_background_page(self):
        try:
            current_url = str(self._page.url or "").casefold()
        except Exception:
            return False
        return bool(
            "securityandbackground" in current_url
            and (
                "complete_securityandbackground" in current_url
                or "node=securityandbackground" in current_url
            )
        )
    ...
    def plan_choice_fields(
        ...
        """
        labels = dict(field_labels or {})
        hints = self._complete_choice_control_hints(
            field_ids,
            control_hints,
        )
                ...
                """token => {
                    ...
                    const matches = Array.from(grouped.values()).filter(items => {
                        ...
                        return identity.includes('otherrelatives')
                            || identity.includes('otherrelativeind')
    ...
    def late_required_validation_probe_fields(
        self,
        field_ids,
        field_labels=None,
        _control_hints=None,
    ):
        """Identify the one CEAC Family field revealed only by validation.

        Some live CEAC sessions accept the immediate-relative answer without
        committing the following Other-Relatives radio group to the DOM.  A
        single guarded Next lets CEAC return its own field-specific validation
        and render that required group.  This hook is intentionally limited to
        the exact lone pending dependent field after its prerequisite has a
        selected value.
        """
        candidates = [
            str(field_id)
            for field_id in field_ids or ()
            if self._is_dependent_family_choice(field_id)
        ]
        if (
            not self._is_relatives_page()
            or len(list(field_ids or ())) != 1
            or len(candidates) != 1
            or not self._family_immediate_choice_answered()
        ):
            return []
        field_id = candidates[0]
        terms = self._family_choice_terms(field_id)
        if not terms or self._family_choice_group(field_id, terms) is not None:
            return []
        return [field_id]
    ...
    def family_date_value_matches(self, field_id, approved_value):
        """Read a Father/Mother date group after a Family postback.

        CEAC preserves an old validation summary until the next submit.  This
        proof is deliberately section-scoped and reconstructs all three live
        controls, so a stale summary cannot reject a correctly repaired DOB.
        """
        if not self._is_relatives_page():
            return None
        rule = self._family_semantic_rule(field_id)
        expected = self._parse_iso_date(approved_value)
        if rule is None or "date" not in rule["kinds"] or expected is None:
            return None
        locator = self._travel_semantic_control(
            rule["terms"],
            "date",
                    ...
                    for (let depth = 0; depth < 8 && current; depth += 1) {
                        ...
                        if (
                            ...
                            return controls.map(item => ({
                                ...
                                text: item.tagName.toLowerCase() === 'select'
        ...
        for item in snapshot:
            if item.get("tag") == "select":
                ...
                if re.fullmatch(r"\d{4}", candidate):
                    year = int(candidate)
        return (year, month, day) == expected

    def work_education1_exact_value_matches(
        self,
        field_id,
        approved_value,
    ):
        """Prove one Work/Education 1 repair despite stale Next errors.

        A manual or guarded submit can leave CEAC's required-field summary on
        the page until the next submit.  That old summary must not reject a
        value that is now present in the exact prompt-scoped control.  This is
        deliberately page- and field-scoped; navigation and unresolved
        controls remain subject to the ordinary strict verifier.
        """
        ...
        if rule is None:
            return None
        approved = str(approved_value or "").strip()

        # Prefer the exact action-owned marker that produced the current
        # verification observation.  CEAC can leave a target validator visible
        # after the value is corrected; re-running the broad prompt binder at
        # this point may resolve a neighbouring address control and falsely
        # pause.  The marker is unique, page-local, and is the same source used
        # by deterministic pending-action recovery on the next run.
        selector = self._field_selectors.get(str(field_id or ""))
        if selector:
            try:
                live_value = self._live_control_value(
                    str(field_id or ""),
                    selector,
            ...
            if live_value is not None:
                ...
                if normalize(live_value) == normalize(approved):
                    return True
                if (
                    "select_text" in rule["kinds"]
                    and self._choice_matches(approved, live_value)
                ):
                    return True
        if "date" in rule["kinds"]:
            expected = self._parse_iso_date(approved)
            locator = self._travel_semantic_control(
                rule["terms"],
                "date",
                section="",
            ...
            if expected is None or locator is None:
                return None
            try:
                snapshot = locator.evaluate(
                    """el => {
        ...
        wants_dna = bool(
            "does_not_apply" in rule["kinds"]
            and self._address_phone_dna_requested(approved)
        )
        kind = (
            "does_not_apply"
            if wants_dna
                ...
                "select_text"
                if "select_text" in rule["kinds"]
                else (
                    "textarea"
                    if "textarea" in rule["kinds"]
                    else "text"
                )
            )
        )
        ...
        try:
            ...
            return None
        if wants_dna:
            return bool(snapshot.get("checked"))
        if snapshot.get("tag") == "select":
            return self._choice_matches(
                approved,
                " ".join((
        ...
        return normalize(snapshot.get("value")) == normalize(approved)
    ...
    def _plan_missing_travel_branch_replay(
        ...
        secondary_id = next((
            ...
            if semantic_field_id_endswith(
                field_id, ".travel.purpose.secondary"
            )
        ), "")
        if (
            not secondary_id
    ...
    def _plan_work_page_once(self, field_ids, labels, hints):
        ...
        # A restored CEAC page can visibly retain BUSINESS while omitting its
        # server-rendered employer panel.  Previously the guarded controller
        # replay was armed only on the next run, after the first action had
        # already paused.  Arm that same existing replay immediately when the
        # exact controller is bindable and at least one required dependent is
        # still missing.  A fresh placeholder selection remains on the normal
        # path because execute() independently proves the current option
        # before it permits the reset/repost sequence.
        primary_action = next((
            action for action in scoped_actions
            if semantic_field_id_endswith(
                action.field_id,
                ".work.primary_occupation",
            )
        ...
        if (
            ...
            primary_action.reason = (
                "V2 CEAC Work first-pass missing-branch controller replay "
                f"[field_id={primary_action.field_id}]"
            )
            self._v2_forced_postback_field_ids.add(primary_action.field_id)
        return scoped_actions, [*generic_unresolved, *scoped_unresolved]
    ...
    def _work_semantic_rule(field_id):
        normalized = str(field_id or "").casefold()
        # Current bridge manifests encode the single present-employer address
        # as ``address.0.<field>`` while the original V2 descriptors used
        # ``address.record.<field>``.  They identify the same CEAC controls.
        # Canonicalize only this exact Work/Education 1 path so the established
        # prompt-scoped rules, stale-error proof, and presence guard all apply
        # to both manifest generations.
        normalized = re.sub(
            r"(\.work\.present\.address\.)\d+(\.)",
            r"\1record\2",
            normalized,
        )
        rules = (
    ...
    def travel_purpose_matches_approved(self, field_id, approved):
        ...
        if not self._is_travel_page():
            return False
        normalized_id = semantic_field_id(field_id)
...
class HumanFirstFastVisibleChromeBrowser(FastVisiblePlaywrightBrowser):
    """Official Chrome that remains detached until explicit human consent."""

    human_first = True
    ALLOWED_CEAC_HOSTS = frozenset({"ceac.state.gov"})

    def __init__(self, config):
        super().__init__(config)
        self._job_id = ""
        self._chrome_process = None
        self._human_first_phase = "BROWSER_STARTING"
        self._human_first_attached = False
        self._attach_error = ""

    def set_job_id(self, job_id):
        if self._chrome_process is not None:
            raise RuntimeError("Chrome job id must be configured before start")
        self._job_id = str(job_id or "")

    def start(self, url):
        """Start official Chrome without importing or starting Playwright."""
        if self._native_input is None:
            raise self._native_input_error or NativeInputUnavailable(
                "V2 系统级鼠标键盘通道不可用。"
            )
        self._native_input.require_authorized()
        if self._chrome_process is not None:
            if not self._chrome_process.is_alive:
                raise ChromeProcessError("The job-owned official Chrome exited")
            return self.runtime_info()
        if not self._job_id:
            raise RuntimeError("Chrome job id was not configured")
        if self._profile_dir is None:
            raise RuntimeError("Chrome profile was not configured")
        self._human_first_phase = "BROWSER_STARTING"
        self._chrome_process = ChromeProcessManager(
            job_id=self._job_id,
            profile_dir=self._profile_dir,
            executable=os.environ.get("DOCFLOW_CHROME_EXECUTABLE"),
            port_min=int(os.environ.get("DOCFLOW_CDP_PORT_MIN", "9300")),
            port_max=int(os.environ.get("DOCFLOW_CDP_PORT_MAX", "9399")),
        )
        try:
            self._chrome_process.start(url)
        except Exception:
            self._human_first_phase = "FAILED"
            raise
        self.browser_launch_source = "official-chrome-stable-human-first"
        self._human_first_phase = "WAITING_HUMAN_ENTRY"
        return self.runtime_info()

    @classmethod
    def _allowed_ceac_page(cls, page):
        try:
            hostname = (urlsplit(str(page.url or "")).hostname or "").casefold()
        except Exception:
            return False
        return hostname in cls.ALLOWED_CEAC_HOSTS

    def _existing_ceac_pages(self):
        pages = []
        for context in list(getattr(self._browser, "contexts", ()) or ()):
            for page in list(getattr(context, "pages", ()) or ()):
                try:
                    if not page.is_closed() and self._allowed_ceac_page(page):
    ...
    def attach_existing_chrome(self):
        """Attach to exactly one existing formal CEAC tab without navigation."""
        manager = self._chrome_process
        if manager is None or not manager.is_alive:
            raise ChromeProcessError("The job-owned official Chrome is not running")
        self._human_first_phase = "ATTACHING_EXECUTOR"
        self._attach_error = ""
        if self._browser is None:
            try:
                ...
                raise ProviderNotConfigured(
                    "Playwright is required only for the post-verification attach"
                ) from error
            self._playwright = sync_playwright().start()
            try:
                # The endpoint is localhost-only and is never returned by an
                # HTTP API, event, or log payload.
                ...
                self._browser = None
                self._human_first_phase = "WAITING_HUMAN_ENTRY"
                self._attach_error = "无法连接当前 Chrome；人工会话仍保留"
                raise ChromeProcessError(self._attach_error)

        candidates = self._existing_ceac_pages()
        if len(candidates) != 1:
            self._human_first_phase = "WAITING_HUMAN_CHECKPOINT"
            self._attach_error = (
                "请只保留一个 CEAC 标签页并进入正式 DS-160 表格"
            )
            raise ChromeProcessError(self._attach_error)
        context, page = candidates[0]
        self._context = context
        self._page = page
        self._configure_timeout_target(context)
        self._configure_timeout_target(page)
        classification = self._classify_live_page(page)
        if str(getattr(classification, "kind", "") or "") != "formal":
            self._human_first_phase = "WAITING_HUMAN_CHECKPOINT"
            self._attach_error = (
                "当前 CEAC 页面还不是可填写的正式表格；请完成人工步骤后重试"
            )
            raise ChromeProcessError(self._attach_error)

        if self._visual_execution:
            context.add_init_script(script=self._visual_document_init_script())
            self._install_visual_document_guard()
        self.focus()
        if self._visual_execution:
            self._ensure_visible_cursor()
        self._human_first_attached = True
        self._human_first_phase = "RUNNING_AUTOFILL"
        return self.runtime_info()
    ...
    def runtime_info(self):
        ...
        process = (
            ...
            if manager is not None
                ...
                "jobId": self._job_id,
                "pid": 0,
                "profileDir": str(self._profile_dir or ""),
                "executable": str(
                    os.environ.get("DOCFLOW_CHROME_EXECUTABLE") or ""
                ),
                "cdpHost": "127.0.0.1",
                "chromeAlive": False,
                "cdpReady": False,
        ...
        return {
            **process,
            ...
            "humanFirst": True,
            "phase": self._human_first_phase,
            "executorAttached": self._human_first_attached,
            "attachError": self._attach_error,
            "browserLaunchSource": self.browser_launch_source,
        }
    ...
    def close(self):
        ...
        self._chrome_process = None
        self._human_first_attached = False
        self._human_first_phase = "COMPLETED"

    def emergency_close(self):
        manager = self._chrome_process
        if manager is not None:
            return manager.stop(timeout=1.0)
        return super().emergency_close()
