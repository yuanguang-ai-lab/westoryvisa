...
from .browser import (
    ...
    HumanFirstFastVisibleChromeBrowser,
)
from .country_executors import executor_class_for_worker
...
def build_fast_service(config=None, registry=None, checkpoint_store=None):
    ...
    human_first_runtime = (
        str(getattr(config, "browser_runtime", "") or "")
        == "chrome_stable_human_first"
    )
    human_first_enabled = bool(
        getattr(config, "human_first_bootstrap", False)
    )
    if human_first_runtime != human_first_enabled:
        raise ValueError(
            "DOCFLOW_BROWSER_RUNTIME and DOCFLOW_HUMAN_FIRST_BOOTSTRAP "
            "must enable the human-first runtime together"
        )
    ...
    def runtime_factory(job, startup_control=None):
        ...
        for attempt in range(2):
            browser = (
                HumanFirstFastVisibleChromeBrowser(config.browser)
                if human_first_runtime
                else provider_registry.create("browser", config.browser)
            )
            if browser is None:
                raise ProviderNotConfigured(
                    "Both computer-use and browser providers are required"
            ...
            set_job_id = getattr(browser, "set_job_id", None)
            if callable(set_job_id):
                set_job_id(job.id)
