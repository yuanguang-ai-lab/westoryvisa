...
from .country_executors import executor_metadata
...
class V2Handler(Handler):
    ...
    def _browser_diagnostics(self, job_id):
        ...
        def capture(computer_use_agent):
            ...
            runtime_info = getattr(browser, "runtime_info", None)
            if callable(runtime_info):
                snapshot = runtime_info()
                if (
                    snapshot.get("humanFirst")
                    and not snapshot.get("executorAttached")
                ):
                    raise ValueError(
                        "Browser diagnostics are disabled before human entry confirmation"
                    )
            ...
            return {
                ...
                "nativeSelectDiagnostics": (
                    browser.native_select_diagnostics()
                    if callable(getattr(
                        browser,
                        "native_select_diagnostics",
                        None,
                    ))
                    else {}
                ),
    ...
    def do_GET(self):
        ...
        if path == "/health":
            ...
            chrome_path = Path(str(
                os.environ.get("DOCFLOW_CHROME_EXECUTABLE")
                or "/usr/bin/google-chrome-stable"
            )).expanduser()
            human_first = bool(
                payload.get("browserRuntime")
                == "chrome_stable_human_first"
                and payload.get("humanFirstReady")
            )
            payload.update({
                ...
                "selectInputBackend": native_input["backend"],
                "selectInputReady": native_input["authorized"],
                "selectInputReason": native_input["reason"],
                # Keep the backend's existing readiness keys so the DocFlow
                # UI does not misreport V2's required trusted-input channel.
                "nativeInputReady": native_input["authorized"],
                "nativeInput": native_input,
                "countryExecution": executor_metadata(),
                "chromeStableInstalled": bool(
                    chrome_path.is_file() and os.access(chrome_path, os.X_OK)
                ),
                "chromeExecutable": str(chrome_path) if human_first else "",
                "cdpAttachReady": bool(
                    not human_first
                    or (
                        chrome_path.is_file()
                        and os.access(chrome_path, os.X_OK)
                    )
                ),
            })
            if "ready" in payload:
                payload["ready"] = bool(
                    payload["ready"]
                    and native_input["authorized"]
                    and payload["cdpAttachReady"]
                )
