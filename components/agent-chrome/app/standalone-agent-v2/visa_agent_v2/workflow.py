...
class FastComputerUseAgent(ComputerUseAgent):
    ...
    def _branch_controller_repair_limit(field_id, default_limit):
        ...
        """
        if semantic_field_id_endswith(field_id, (
            ".travel.arrivaldate",
            ".travel.stayduration",
        )):
    ...
    def _field_ids_from_errors(errors, allowed_field_ids):
        ...
        dependent_ids = [
            ...
            if semantic_field_id_endswith(
                field_id, ".family.other_relatives_us"
            )
        ]
        course_ids = [
            field_id for field_id in allowed
        ...
        home_postal_ids = [
            field_id for field_id in allowed
            if semantic_field_id_endswith(
                field_id, ".address_phone.contact.homepostalcode"
            )
        ]
        matched = []
        has_unscoped = False
    ...
    def _apply_browser_action_postcondition(self, action, verification):
        ...
        if (
            ...
            and ".work_education1." in field_id.casefold()
            ...
            prove = getattr(
                self.browser,
                "work_education1_exact_value_matches",
                None,
            )
            try:
                live_match = bool(
    ...
    def _stale_completed_page_fields(
        ...
        if callable(purpose_match):
            ...
            for field_id in candidates:
                if not semantic_field_id_endswith(field_id, (
        ...
        # Passport, Family and Work/Education pages are identity and branch
        # boundaries.
        # Never let a
        ...
        if strict:
            ...
            job.record(
                ...
                "Strict-page Next remained locked because one or more "
