...
class BuiltinAdapterTests(unittest.TestCase):
            ...
            driver._page.set_content(
                ...
                <script>
                  ...
                  window.renderDob = () => {
                    ...
                    host.innerHTML = `
                      <select id="DOB_DAY" name="DOB_DAY">${days}</select>
                      <select id="DOB_MONTH" name="DOB_MONTH">${months}</select>
                      <input id="DOB_YEAR" name="DOB_YEAR"
                             value="${state.year}" maxlength="4">
                    `;
                    host.querySelector("#DOB_DAY").addEventListener(
                      "change",
                      event => {
                    ...
                    host.querySelector("#DOB_MONTH").addEventListener(
                      "change",
                      event => {
                    ...
                    host.querySelector("#DOB_YEAR").addEventListener(
                      "input",
                      event => {
    ...
    def test_profile_start_removes_only_dead_chromium_singleton_links(self):
        with tempfile.TemporaryDirectory() as directory:
            profile = Path(directory) / "agent-profile"
            profile.mkdir()
            for name, target in (
                ("SingletonLock", "old-container-43210"),
                ("SingletonCookie", "123456789"),
                ("SingletonSocket", "/tmp/old-container/SingletonSocket"),
            ):
                (profile / name).symlink_to(target)
            dead_process = mock.Mock(returncode=1, stdout="")
            driver = PlaywrightBrowserDriver(provider("playwright"))

            with mock.patch(
                "visa_agent.adapters.subprocess.run",
                return_value=dead_process,
            ):
                driver.set_profile_dir(profile)
    ...
    def test_profile_start_preserves_live_profile_owned_singletons(self):
        with tempfile.TemporaryDirectory() as directory:
            profile = Path(directory) / "agent-profile"
            profile.mkdir()
            for name, target in (
                ("SingletonLock", "current-container-43210"),
                ("SingletonCookie", "123456789"),
                ("SingletonSocket", "/tmp/current/SingletonSocket"),
            ):
                (profile / name).symlink_to(target)
