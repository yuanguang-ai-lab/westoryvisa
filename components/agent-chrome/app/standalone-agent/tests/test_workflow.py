...
class LateValidationProbeBrowser(MultiPageNavigationBrowser):
    ...
    def plan_fields(self, field_ids, _labels, _hints):
        if not self.revealed:
            return [], list(field_ids)
        return super().plan_fields(field_ids, _labels, _hints)
    ...
    def plan_next(self):
        return ComputerAction(
            kind=ActionKind.CLICK,
            target_hint="Next: Work/Education/Training",
            reason="Deterministic fixed CEAC Next control",
        )
    ...
    def execute(self, action):
        if action.kind == ActionKind.CLICK and not self.revealed:
            ...
            self.revealed = True
            self.validation_errors = [
                f"Other relatives is required [field_id={self.field_id}]"
            ]
            return
        if action.kind == ActionKind.CLICK:
            self.validation_errors = []
            ...
            self.title = "Present Work/Education/Training Information"
            self.visible_text = self.title
...
class WorkflowTests(unittest.TestCase):
    ...
    def test_system_family_validation_probe_can_reveal_required_field(self):
        field_id = "ceac.relatives.family.other_relatives_us"
        browser = LateValidationProbeBrowser(field_id)
        job = AgentJob(
            fields=[field(field_id, "no")],
            start_url=browser.url,
        ...
        self.assertNotEqual(result.state, JobState.BLOCKED)
        self.assertGreaterEqual(len(browser.executed), 2)
        self.assertEqual(browser.executed[0].kind, ActionKind.CLICK)
        self.assertEqual(browser.executed[1].field_id, field_id)
        self.assertTrue(any(
    ...
    def test_provider_retry_budget_is_not_multiplied_by_workflow_retry(self):
    ...
    def test_docflow_manual_page_boundary_stops_before_next(self):
        browser = MultiPageNavigationBrowser()
        field_id = "ceac.personal1.001.personal.surname"
        job = AgentJob(
            fields=[field(field_id, "XIA")],
            ...
            required_field_ids=[field_id],
            auto_next=True,
            manual_review_page_plan_ids=["ceac-plan-personal1"],
        )
        ...
        self.assertIn("点击 Next 前暂停", result.human_checkpoint)
        self.assertEqual(
            [action.kind for action in browser.executed],
            [ActionKind.TYPE],
        )
        self.assertTrue(any(
            event.kind == "manual_review_page_next_blocked"
            for event in result.events
        ))
