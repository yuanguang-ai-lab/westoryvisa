...
class AgentConfig:
    ...
    # The default remains the existing Playwright-owned Chromium lifecycle.
    # A parallel worker can opt into the official-Chrome human-first runtime
    # without changing any existing worker or checkpoint schema.
    browser_runtime: str = "playwright_chromium"
    human_first_bootstrap: bool = False
...
def load_config(environ=None):
    ...
    return AgentConfig(
        ...
        browser_runtime=(
            env.get("DOCFLOW_BROWSER_RUNTIME", "playwright_chromium").strip()
            or "playwright_chromium"
        ),
        human_first_bootstrap=_truthy(
            env.get("DOCFLOW_HUMAN_FIRST_BOOTSTRAP"), False
        ),
