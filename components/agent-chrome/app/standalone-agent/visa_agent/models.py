...
class AgentJob:
    ...
    # Reviewed DocFlow pages can contain sensitive or incomplete branches
    # that must never be crossed automatically.  This boundary is separate
    # from field permissions because those questions may intentionally have
    # no provider field at all.
    manual_review_page_plan_ids: List[str] = field(default_factory=list)
...
def job_from_primitive(payload):
    ...
    return AgentJob(
        ...
        manual_review_page_plan_ids=[
            str(item)
            for item in payload.get("manual_review_page_plan_ids") or []
        ],
