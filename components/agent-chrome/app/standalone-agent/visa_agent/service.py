...
class AgentService:
    ...
    def health(self):
        ...
        browser_runtime = str(
            getattr(self.config, "browser_runtime", "playwright_chromium")
            or "playwright_chromium"
        )
        human_first = bool(
            getattr(self.config, "human_first_bootstrap", False)
            and browser_runtime == "chrome_stable_human_first"
        )
        return {
            "ok": True,
    ...
    def create_job(self, payload):
        ...
        manual_review_page_plan_ids = payload.get(
            "manualReviewPagePlanIds", []
        )
        if not isinstance(manual_review_page_plan_ids, list) or any(
            not re.fullmatch(r"ceac-plan-[a-z0-9_]+", str(item or ""))
            for item in manual_review_page_plan_ids
        ):
            raise ServiceError("manualReviewPagePlanIds is invalid")
        job.manual_review_page_plan_ids = list(dict.fromkeys(
            str(item) for item in manual_review_page_plan_ids
        ))[:32]
        store.save(job)
    ...
    def get_job(self, job_id):
        ...
        try:
            ...
            payload["browser_runtime"] = str(
                getattr(self.config, "browser_runtime", "")
                or "playwright_chromium"
            )
            payload["human_first"] = bool(
                getattr(self.config, "human_first_bootstrap", False)
            )
        finally:
    ...
    def sync_job(self, job_id, payload):
        ...
        with self._job_lifecycle_lock(job_id):
            ...
            old_manual_review_page_plan_ids = list(
                job.manual_review_page_plan_ids
            )
            old_state = job.state
            ...
            snapshot_changed = bool(
                ...
                or old_manual_review_page_plan_ids
                != list(job.manual_review_page_plan_ids)
            )
            if snapshot_changed:
            ...
            if preserve_hard_boundary:
                ...
                should_wake_resume = False
                self._clear_automatic_retry_state(job)
            if old_wait_kind == "user_paused":
                # Updating the approved DocFlow snapshot must never be an
                # implicit resume.  The operator remains in sole control of
                # the retained browser until they press Continue Gemini.
                job.state = JobState.WAITING_HUMAN
                job.wait_kind = "user_paused"
                job.human_checkpoint = old_human_checkpoint or (
                    "Gemini 已由用户暂停；保留当前浏览器"
                )
                job.continuous_run_requested = False
                job.sync_resume_pending = False
                should_wake_resume = False
    ...
    def pause_job(self, job_id, payload):
        """Fence the current run while retaining its exact browser session.

        Pausing is deliberately stronger than publishing a UI flag.  Revoking
        the execution lease makes every action planned by the old worker stale,
        and advancing the durable generation prevents a late checkpoint from
        reviving that worker.  The pending action is discarded because it may
        have been planned against the page before the operator navigated Back.
        """
        actor = str((payload or {}).get("actor") or "api").strip()[:120]
        with self._job_lifecycle_lock(job_id):
            job = self._load_job(job_id)
            if job.state in TERMINAL_JOB_STATES:
                raise ServiceError(
                    f"Job cannot pause from terminal state: {job.state.value}",
                    409,
                )
            if job.wait_kind == "user_paused":
                return _public_job_payload(job)
            ...
            self._clear_automatic_retry_state(job)
            job.human_checkpoint = (
                "Gemini 已由用户暂停；保留当前浏览器，可人工修改后继续"
            )
            job.record(
                "user_pause_requested",
                job.human_checkpoint,
                actor=actor,
                discardedPendingActionId=str(
                    getattr(pending, "id", "") or ""
                ),
                discardedPendingActionKind=str(
                    getattr(getattr(pending, "kind", ""), "value", "")
                    or getattr(pending, "kind", "")
                    or ""
                ),
            ...
            self._require_store().save(job)
        # Wake any watcher currently sleeping so it re-reads the disarmed
        # durable state instead of waiting for its previous deadline.
        self._wake_continuous_resume(job_id)
        return _public_job_payload(job)
    ...
    def open_job(self, job_id):
        ...
        with self._job_lifecycle_lock(job_id):
            ...
            if (
                ...
                job.state = JobState.WAITING_HUMAN
                if bool(getattr(self.config, "human_first_bootstrap", False)):
                    job.wait_kind = "human_form_entry"
                    job.human_checkpoint = (
                        "浏览器已打开。请完全人工完成人机验证、登录和"
                        "申请恢复；进入 DS-160 正式表格后再确认接管。"
                    )
                else:
            ...
            if bool(getattr(self.config, "human_first_bootstrap", False)):
                job.record(
                    "official_chrome_started",
                    "Job-owned official Chrome Stable started without Playwright",
                )
                job.record(
                    "waiting_human_entry",
                    "Waiting for explicit human confirmation before executor attach",
                )
            self._require_store().save(job)
    ...
    def browser_runtime(self, job_id):
        """Return redacted process state without inspecting the live page."""
        self._load_job(job_id)
        with self._runtime_lock:
            runtime = self._runtimes.get(job_id)
        ...
        def snapshot(computer_use_agent):
            browser = computer_use_agent.browser
            info = getattr(browser, "runtime_info", None)
            if callable(info):
                return info()
            return {
                "jobId": job_id,
                ...
                "humanFirst": False,
                "phase": "RUNNING_AUTOFILL",
                "chromeAlive": True,
                "executorAttached": True,
            }

        try:
            return runtime.try_call(snapshot, timeout=3)
        except _RuntimeBusy:
            return {
                "jobId": job_id,
                "runtime": str(
    ...
    def confirm_form_entry(self, job_id):
        """Attach the executor only after the operator confirms form entry."""
        if not bool(getattr(self.config, "human_first_bootstrap", False)):
            # Existing Chromium workers keep their historical start behavior;
            # the new endpoint is harmless and idempotent during mixed deploys.
            return _public_job_payload(self._load_job(job_id))
        with self._job_lifecycle_lock(job_id):
            job = self._load_job(job_id)
            if job.state in TERMINAL_JOB_STATES:
                raise ServiceError(
                    f"Job cannot attach from terminal state: {job.state.value}",
                    409,
                )
            with self._runtime_lock:
                runtime = self._runtimes.get(job_id)
            if runtime is None or not runtime.is_available:
                raise ServiceError("请先打开当前任务的官方 Chrome", 409)

            def runtime_snapshot(computer_use_agent):
                info = getattr(
            ...
            try:
                existing_attach = runtime.try_call(
                    runtime_snapshot,
                    timeout=3,
                )
            except _RuntimeBusy as error:
                raise ServiceError("填写执行层正在连接，请稍后重试", 409) from error
            if (
                job.state == JobState.READY_FOR_FORM
                and existing_attach.get("executorAttached") is True
            ):
                return {
                    **_public_job_payload(job),
                    "browser_runtime": existing_attach,
                }
            job.record(
                "human_entry_confirmed",
                "The operator confirmed entry into the formal DS-160 form",
            )
            job.record(
                "executor_attach_started",
                "Executor attach started after explicit human confirmation",
            )
            self._require_store().save(job)

            def attach(computer_use_agent):
                attach_existing = getattr(
                    computer_use_agent.browser,
                    "attach_existing_chrome",
                    None,
                )
                if not callable(attach_existing):
                    raise RuntimeError("Human-first browser attach is unavailable")
                return attach_existing()

            try:
                attach_info = runtime.call(attach, timeout=25)
            except Exception as error:
                job = self._load_job(job_id)
                job.state = JobState.WAITING_HUMAN
                job.wait_kind = "human_form_entry"
                job.human_checkpoint = str(error)[:400]
                job.record(
                    "executor_attach_failed",
                    "Executor attach was rejected; the official Chrome session remains open",
                    reason=type(error).__name__,
                )
                self._require_store().save(job)
                if isinstance(error, ServiceError):
                    raise
                raise ServiceError(str(error)[:400], 409) from error
            job = self._load_job(job_id)
            job.state = JobState.READY_FOR_FORM
            job.wait_kind = ""
            job.human_checkpoint = None
            job.record(
                "executor_attach_succeeded",
                "Executor attached to the existing formal CEAC page",
                sameProcess=bool(attach_info.get("chromeAlive")),
                executorAttached=bool(attach_info.get("executorAttached")),
            )
            self._require_store().save(job)
            return {
                **_public_job_payload(job),
                "browser_runtime": attach_info,
            }
    ...
    def start_job(self, job_id, expected_watcher_epoch=None):
        ...
        with lifecycle_lock:
            ...
            if bool(getattr(self.config, "human_first_bootstrap", False)):
                with self._runtime_lock:
                    human_first_runtime = self._runtimes.get(job_id)
                if (
                    job.wait_kind == "human_form_entry"
                    or human_first_runtime is None
                    or not human_first_runtime.is_available
                ):
                    raise ServiceError(
                        "请先在当前官方 Chrome 中进入正式表格并确认接管",
                        409,
                    )

                def attached(computer_use_agent):
                    info = getattr(
                        computer_use_agent.browser,
                        "runtime_info",
                        None,
                    ...
                    return bool(
                        callable(info)
                        and info().get("executorAttached")
                    )

                try:
                    executor_attached = human_first_runtime.try_call(
                        attached,
                        timeout=3,
                    )
                except _RuntimeBusy as error:
                    raise ServiceError(
                        "当前 Chrome 正在完成上一个接管操作，请稍后重试",
                        409,
                    ) from error
                if not executor_attached:
                    raise ServiceError(
                        "填写执行层尚未连接；请先确认已进入正式表格",
                        409,
                    )
            if job.wait_kind == "user_paused":
                if expected_watcher_epoch is not None:
                    raise ServiceError(
                        "A user-paused job requires an explicit resume",
                        409,
                    )
                previous_checkpoint = str(job.human_checkpoint or "")[:400]
                job.wait_kind = ""
                job.human_checkpoint = None
                job.sync_resume_pending = False
                self._clear_automatic_retry_state(job)
                job.record(
                    "user_pause_resumed",
                    "The user explicitly resumed Gemini from the live page",
                    previousCheckpoint=previous_checkpoint,
                    pendingActionPreserved=False,
                )
            if job.wait_kind == "manual_hard_boundary":
...
class Handler(BaseHTTPRequestHandler):
    ...
    JOB_PATH = re.compile(
        ...
        r"(?:/(?P<action>review|sync|open|start|resume|pause|cancel|"
        r"confirm-form-entry|resume-after-human|browser-runtime))?$"
    )
    ...
    def do_GET(self):
        ...
        if matched and matched.group("action") == "browser-runtime":
            return self._call(
                self.service.browser_runtime, matched.group("job_id")
            )
    ...
    def do_POST(self):
        ...
        if matched:
            ...
            if action in {"confirm-form-entry", "resume-after-human"}:
                return self._call(self.service.confirm_form_entry, job_id)
            if action in {"start", "resume"}:
                return self._call(self.service.start_job, job_id)
            if action == "pause":
                return self._call(
                    self.service.pause_job, job_id, self.read_json()
                )
            if action == "cancel":
                return self._call(
    ...
    def require_api_token(self):
        expected = os.environ.get("DOCFLOW_AGENT_API_TOKEN", "").strip()
        if not expected:
            return True
        ...
        if (
            ...
            and hmac.compare_digest(candidate.strip(), expected)
        ):
            return True
        self.json_response({"error": "Agent API authorization required"}, 401)
        return False
