...
class ComputerUseAgent:
    ...
    def run(self, job):
        ...
        while run_step_count < self.max_steps:
            ...
            plan_source = ""
            # CEAC occasionally withholds one required dependent Family radio
            # until the page is submitted once.  A V2 browser may opt into a
            # narrowly-scoped, read-only eligibility hook; the workflow then
            # performs one durable Next probe and relies on ordinary CEAC
            # validation mapping to reopen and fill the revealed field.  The
            # recorded probe prevents repeated submission loops.
            late_probe = getattr(
                self.browser,
                "late_required_validation_probe_fields",
                None,
            )
            if pending_page_fields and callable(late_probe):
                try:
                    probe_fields = [
                        str(field_id)
                        for field_id in late_probe(
                            pending_page_fields,
                            field_labels,
                            control_hints,
                        ) or ()
                        if str(field_id) in set(pending_page_fields)
                    ]
                except Exception as error:
                    probe_fields = []
                    job.record(
                        "late_required_validation_probe_unavailable",
                        "The guarded dependent-field validation probe could "
                        "not be evaluated; normal planning remained active",
                        errorType=type(error).__name__,
                        pagePlanId=page_plan.id,
                    )
                prior_probe = any(
                    event.kind == "late_required_validation_probe_dispatched"
                    and str(event.detail.get("pagePlanId") or "")
                    == str(page_plan.id or "")
                    and set(event.detail.get("fieldIds") or ())
                    == set(probe_fields)
                    for event in job.events or ()
                )
                if probe_fields and not prior_probe:
                    plan_next = getattr(self.browser, "plan_next", None)
                    try:
                        next_action = (
                            plan_next() if callable(plan_next) else None
                        )
                    except Exception:
                        next_action = None
                    if next_action is not None:
                        actions = [next_action]
                        plan_source = "deterministic-validation-probe"
                        job.record(
                            "late_required_validation_probe_dispatched",
                            "One guarded Next was planned so CEAC could render "
                            "its withheld required dependent field",
                            pagePlanId=page_plan.id,
                            fieldIds=probe_fields,
                        )
                        self._save(job)
            ...
            if (
                not actions
                and callable(local_planner)
                and local_pending_page_fields
            ):
            ...
            if not pending_page_fields and not actions:
                ...
                if stale_page_fields:
                    ...
                    self._save(job)
                    continue
                if page_plan.id in set(
                    job.manual_review_page_plan_ids or ()
                ):
                    job.record(
                        "manual_review_page_next_blocked",
                        "Next remained locked at a reviewed DocFlow manual "
                        "page boundary",
                        pagePlanId=page_plan.id,
                    )
                    return self._wait_human(
                        job,
                        "本页包含必须由顾问按客户真实情况确认的问题；"
                        "Gemini 已在点击 Next 前暂停。请人工确认并进入下一页，"
                        "任务会从新页面继续。",
            ...
            if not actions:
                ...
                try:
                    ...
                    if (
                        ...
                        and self._is_retryable_provider_exhaustion(error)
                    ):
            ...
            for action_number, action in enumerate(actions, start=1):
                ...
                system_owned_validation_probe = bool(
                    action.kind == ActionKind.CLICK
                    and not action.field_id
                    and self._is_next_action(action)
                    and plan_source == "deterministic-validation-probe"
                    and pending_page_fields
                )
                system_owned_next = bool(
                    ...
                    and (
                        ...
                        or system_owned_validation_probe
                    )
                )
                if (
                    system_owned_next
                ...
                if system_owned_next and len(actions) != 1:
                    return self._block(
                        job,
                ...
                is_next_navigation = bool(
                    system_owned_next and self._is_next_action(action)
                )
                    ...
                    if is_next_navigation:
                        ...
                        if repair_fields and not has_unscoped_error:
                            ...
                            for field_id in repair_fields:
                                ...
                                if (
                                    not system_owned_validation_probe
                                    and self._visual_failure_count(
                                        job,
                                        page_plan.id,
                                        field_id,
                                    ) < self.VISUAL_FIELD_FAILURE_LIMIT
