...
class ServiceAndIsolationTests(unittest.TestCase):
    ...
    def test_user_pause_fences_old_next_and_resume_reuses_browser(self):
        with tempfile.TemporaryDirectory() as directory:
            run_started = threading.Event()
            release_stale_worker = threading.Event()
            runtimes = []

            class PausableRuntime(self.FakeRuntime):
                def __init__(runtime_self):
                    super().__init__()
                    runtime_self.attempt = 0

                def run(runtime_self, job):
                    runtime_self.attempt += 1
                    if runtime_self.attempt == 1:
                        job.state = JobState.FILLING_FORM
                        job.pending_action = ComputerAction(
                            kind=ActionKind.CLICK,
                            target_hint="Next: Family",
                            reason="Deterministic fixed CEAC Next control",
                            id="stale-next-before-user-back",
                            execution_generation=job.execution_generation,
                        )
                        runtime_self.checkpoint_store.save(job)
                        run_started.set()
                        release_stale_worker.wait(timeout=2)
                        # This save belongs to the revoked generation and must
                        # never replace the authoritative user-paused state.
                        runtime_self.checkpoint_store.save(job)
                    job.state = JobState.WAITING_HUMAN
                    job.wait_kind = "manual_hard_boundary"
                    job.human_checkpoint = "resumed from current live page"
                    runtime_self.checkpoint_store.save(job)
                    return job
            ...
            paused = service.pause_job(
                reviewed["id"], {"actor": "consultant-1"}
            )
            self.assertEqual(paused["state"], "waiting_human")
            self.assertEqual(paused["wait_kind"], "user_paused")
            self.assertFalse(paused["continuous_run_requested"])
            self.assertIsNone(paused["pending_action"])
            paused_generation = paused["execution_generation"]
            self.assertGreaterEqual(paused_generation, 2)
            with self.assertRaises(ServiceError) as still_quiescing:
                service.start_job(reviewed["id"])
            self.assertEqual(still_quiescing.exception.status, 409)

            release_stale_worker.set()
            start_thread.join(timeout=2)
            self.assertFalse(start_thread.is_alive())
            stored = service.checkpoint_store.load_job(reviewed["id"])
            self.assertEqual(stored.wait_kind, "user_paused")
            self.assertIsNone(stored.pending_action)
            self.assertEqual(
                stored.execution_generation,
                paused_generation,
            )
            self.assertFalse(runtimes[0].browser.closed)
            ...
            self.assertEqual(synchronized["wait_kind"], "user_paused")
            self.assertFalse(synchronized["continuous_run_requested"])

            resumed = service.start_job(reviewed["id"])
            self.assertEqual(resumed["wait_kind"], "manual_hard_boundary")
            self.assertGreater(
                resumed["execution_generation"],
                paused_generation,
            )
            self.assertTrue(any(
                event["kind"] == "user_pause_resumed"
    ...
    def test_create_job_persists_reviewed_manual_page_boundaries(self):
        with tempfile.TemporaryDirectory() as directory:
            service = AgentService(AgentConfig(data_dir=Path(directory)))
            created = service.create_job({
                "startUrl": "https://ceac.state.gov/GenNIV/form",
            ...
            stored = service._require_store().load_job(created["id"])

            self.assertEqual(
                stored.manual_review_page_plan_ids,
                ["ceac-plan-previous_us_travel"],
            )
