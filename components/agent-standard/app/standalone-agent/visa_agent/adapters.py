...
class ProviderRequestError(RuntimeError):
    ...
    def __init__(self, message, *, retryable=None, status_code=None):
    ...
    def request(self, method, url, body=None, headers=None, timeout=120):
        ...
        try:
            ...
            raise ProviderRequestError(
                f"Provider HTTP {error.code}: request rejected",
...
class PlaywrightBrowserDriver:
    ...
    def set_profile_dir(self, path):
        ...
        self._remove_stale_profile_singletons(target)
        self._profile_dir = target
        self._profile_dir_validated = True
        return target

    @staticmethod
    def _profile_lock_has_live_owner(profile, lock):
        """Return True only for a live Chrome using this exact profile."""
        try:
            if not lock.is_symlink():
                return False
            matched = re.search(r"-(\d+)$", str(os.readlink(lock) or ""))
            if not matched:
                return False
            ...
            for index, token in enumerate(tokens):
                ...
                if not candidate:
                    continue
                try:
                    if Path(candidate).expanduser().resolve() == profile:
                        return True
                except OSError:
                    continue
    ...
    @classmethod
    def _remove_stale_profile_singletons(cls, profile):
        """Remove only dead Chromium singleton links from one private profile.

        Browser profiles live on a Docker volume so CEAC cookies survive an
        Agent restart.  Chromium's singleton symlinks survive too, but their
        PID and /tmp socket belong to the old container.  Keeping those dead
        links prevents the recovery browser from starting and leaves noVNC on
        a blank desktop.  Never touch them while an exact profile-owned Chrome
        process is still alive.
        """
        resolved = Path(profile).resolve()
        lock = resolved / "SingletonLock"
        if cls._profile_lock_has_live_owner(resolved, lock):
            return False
        removed = False
        for name in ("SingletonLock", "SingletonCookie", "SingletonSocket"):
            singleton = resolved / name
            try:
    ...
    def constrain_action_value(self, action):
        ...
        if (
            ...
            or not details.get("hasMaxLength")
        ):
            return None
        try:
        ...
        if len(original) <= maximum:
            return None
        normalized = original[:maximum].rstrip()
        if not normalized:
            raise ControlValueConstraintError(
    ...
    def _deterministic_control(self, field_id, labels, hints):
        ...
        if occurrence is not None and explicit_repeated_labels:
            ...
            # identity evidence; use hints only when no such label exists.
            normalized_hints = []
        if normalized_hints:
