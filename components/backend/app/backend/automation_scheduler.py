...
class WorkerLoop:
    ...
    def _register(self, force=False):
        ...
        automation_queue.register_worker(
            ...
            browser_runtime=str(
                readiness.get("browserRuntime") or "playwright_chromium"
            ),
        )
        return self.healthy
    ...
    def _start(self, job):
        if self._run_active(job["id"]):
            return
        if job.get("browser_runtime") == "chrome_stable_human_first":
            try:
                confirmed = confirm_provider_form_entry(
                    job["provider_job_id"], base_url=self.config.api_url
                )
            except AgentClientError as error:
                automation_queue.transition_job(
                ...
                return
            if str(confirmed.get("state") or "") != "ready_for_form":
                automation_queue.transition_job(
                    connect,
                    job_id=job["id"],
                    worker_id=self.config.worker_id,
