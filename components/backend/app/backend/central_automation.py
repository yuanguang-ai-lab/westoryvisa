...
SUPPORTED_BROWSER_RUNTIMES = frozenset({
    "playwright_chromium",
...
def enqueue_ds160_job(
    ...
    *,
    ...
    browser_runtime = str(browser_runtime or "playwright_chromium")
    if browser_runtime not in SUPPORTED_BROWSER_RUNTIMES:
        raise ValueError("自动填写浏览器版本无效")
    ...
    return {
        ...
        "browserRuntime": browser_runtime,
        "humanFirst": browser_runtime == "chrome_stable_human_first",
...
def public_job_status(connect, *, case_id, public_job_id, user):
    ...
    return {
        ...
        "executionMode": "gemini-v2-linux",
        "browserRuntime": str(
            status.get("browserRuntime")
            or job.get("browser_runtime")
            or "playwright_chromium"
        )[:80],
        "humanFirst": bool(
            status.get("humanFirst") is True
            or job.get("browser_runtime") == "chrome_stable_human_first"
        ),
        "viewerUrl": _viewer_url(job, payload, "control"),
...
def runtime_readiness(connect):
    ...
    for item in registered:
        runtime = str(item.get("browser_runtime") or "playwright_chromium")
        summary = browser_runtimes.setdefault(runtime, {
            "ready": True,
            "workerCount": 0,
            "capacity": 0,
            "activeJobs": 0,
        ...
        summary["capacity"] += int(item.get("capacity") or 1)
        summary["activeJobs"] += int(item.get("active_jobs") or 0)
    ...
    return {
        ...
        "capacity": capacity,
        "activeJobs": active,
        "browserRuntimes": browser_runtimes,
        "chromePilotAvailable": bool(
            browser_runtimes.get("chrome_stable_human_first", {}).get("ready")
        ),
        "message": (
