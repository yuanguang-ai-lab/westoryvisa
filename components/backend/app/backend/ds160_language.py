...
def _deepseek_translation(value, context=""):
    """Translate a Chinese DS-160 value through DeepSeek's server-side API."""
    global _DEEPSEEK_DISABLED_UNTIL, _DEEPSEEK_LAST_SUCCESS
    ...
    if not api_key:
        return ""
    with _DEEPSEEK_LOCK:
        if time.monotonic() < _DEEPSEEK_DISABLED_UNTIL:
            return ""
    payload = json.dumps({
        ...
        "messages": [
            {
                "role": "system",
                "content": (
                    "Translate Chinese text into concise factual English suitable for a U.S. "
                    "DS-160 form. Preserve all numbers, dates and identifiers. Use Hanyu Pinyin "
                    "for personal names. Do not add facts. Return JSON only in this exact shape: "
                    "{\"translation\":\"...\"}."
                ),
            },
            {
                "role": "user",
                "content": f"Field context: {context or 'DS-160 free text'}\nText: {value}",
            },
        ],
    ...
    try:
        req = url_request.Request(
            f"{_deepseek_endpoint()}/chat/completions",
            data=payload,
            headers={
        ...
        if not translated or contains_cjk(translated):
            raise ValueError("DeepSeek returned an unusable translation")
        with _DEEPSEEK_LOCK:
            _DEEPSEEK_LAST_SUCCESS = time.monotonic()
            _DEEPSEEK_DISABLED_UNTIL = 0.0
        return translated
    except (OSError, ValueError, KeyError, TypeError, IndexError, url_error.URLError, TimeoutError):
        with _DEEPSEEK_LOCK:
            _DEEPSEEK_DISABLED_UNTIL = time.monotonic() + 120
            _DEEPSEEK_LAST_SUCCESS = 0.0
        return ""
...
def _libretranslate_translation(value, _context=""):
    ...
    provider = _translation_provider()
    if provider not in {"auto", "deepseek", "libre", "libretranslate"}:
        return ""
...
def translation_service_status():
    ...
    result = {
        "provider": provider,
        "deepSeek": False,
        "deepSeekConfigured": bool(api_key),
        "deepSeekModel": _deepseek_model(),
        "libreTranslate": False,
        "ollamaFallback": provider in {"auto", "deepseek", "ollama"},
    }
    if provider in {"off", "none", "disabled"}:
        return result
    if provider in {"auto", "deepseek"} and api_key:
        with _DEEPSEEK_LOCK:
            recently_ready = (
                _DEEPSEEK_LAST_SUCCESS
                and time.monotonic() - _DEEPSEEK_LAST_SUCCESS < 600
            )
        if recently_ready:
            ...
            try:
                ...
                with url_request.urlopen(req, timeout=5.0) as response:
                ...
                result["deepSeek"] = _deepseek_model() in model_ids
                if result["deepSeek"]:
                    with _DEEPSEEK_LOCK:
                        _DEEPSEEK_LAST_SUCCESS = time.monotonic()
                        _DEEPSEEK_DISABLED_UNTIL = 0.0
...
def _ollama_translation(value, context):
    ...
    provider = _translation_provider()
    if provider not in {"auto", "deepseek", "ollama"}:
        return ""
    with _OLLAMA_LOCK:
...
def _model_translation(value, context=""):
    """Try configured translation providers in priority order."""
    for provider, translator in (
        ("deepseek", _deepseek_translation),
        ("libretranslate", _libretranslate_translation),
        ("ollama", _ollama_translation),
    ):
        translated = translator(value, context)
        if translated:
...
def translate_ds160_value(
    ...
    if _should_transliterate(field_id):
        ...
        # Linux production images may not have a local pinyin engine. In that
        # case romanize() deliberately returns the original CJK text; do not
        # pass it into CEAC normalization because that would erase the value.
        if contains_cjk(translated) and allow_model:
            translated, provider = _model_translation(
                original, context or field_id
            )
            review_required = True
    else:
        translated = ""
    ...
    if translated:
        ...
        normalized_fallback = normalize_ceac_text(fallback)
        result = {
            # Retaining an explicitly review-required original is safer than
            # silently converting a customer's non-Latin name into an empty
            # string when every translation provider is unavailable.
            "value": normalized_fallback or original,
            "originalValue": original,
            "provider": provider if normalized_fallback else "translation_required",
            "reviewRequired": True if not normalized_fallback else (
                review_required or contains_cjk(fallback)
            ),
        }
    if not result["value"]:
