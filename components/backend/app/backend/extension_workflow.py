...
CEAC_PAGE_URLS = {
    ...
    **{
        f"security_background{part}": (
            ...
            f"node=SecurityandBackground{part}"
        )
...
def _ceac_action_value(action_id, label, kind, value):
    ...
    if (
        ...
        and re.search(
            r"(?:phone|telephone)",
            f"{action_id or ''} {label or ''}",
            re.IGNORECASE,
        )
    ):
        # CEAC rejects telephone values that retain OCR/display grouping
        # spaces. Preserve meaningful punctuation such as ``+`` and ``-``
        # while removing every Unicode whitespace character.
...
def _action(action_id, label, kind, value, **extra):
    ...
    if kind == "does_not_apply" and "causesRefresh" not in extra:
        action["causesRefresh"] = True
...
def is_does_not_apply_value(value):
    return _clean(value).upper() in {
        "D", "DNA", "DOES NOT APPLY", "DO NOT KNOW", "NOT APPLICABLE",
        "N/A", "NONE", "UNKNOWN",
    }


def normalize_ceac_address_lines(line1, line2="", maximum=40):
    """Split a reviewed address across CEAC's two retained address rows."""
    first_source = _clean(line1, 500).strip(" ,")
    second_source = _clean(line2, 500).strip(" ,")
    first, overflow = split_address_line1_first(first_source, maximum)
    if overflow:
        second_source = ", ".join(
            item for item in (overflow, second_source) if item
        )
    second, _discarded = split_address_line1_first(second_source, maximum)
    return first, second
...
def build_travel_actions(payload):
    ...
    if duration:
        actions.append(_action(
            ...
            controlHints=["TRAVEL_LOS", "LENGTH_OF_STAY", "LENGTH_STAY"],
            duration=duration,
        ))
    ...
    if address["street2"].upper() in {
        "NONE", "N/A", "NA", "DNA", "NOT APPLICABLE", "DOES NOT APPLY",
    }:
        address["street2"] = ""
    ...
    payer_details_by_answer = {
        "other_person": {
            "surname", "givenNames", "phone", "email", "relationship", "address",
        },
        # CEAC reuses the present-employer data collected later and does not
        # expose payer detail controls for this choice on the Travel page.
        "present_employer": set(),
        "us_employer": {"phone", "organization", "address"},
        "other_organization": {"phone", "organization", "address"},
    }
    allowed_payer_details = payer_details_by_answer.get(payer_answer, set())
    if allowed_payer_details:
        for detail_id, action_id, label, kind, terms in payer_detail_specs:
            if detail_id not in allowed_payer_details:
                continue
...
QUESTION_PAGE_KEYS = {
    ...
    "security.communicable_disease": "security_background1",
    "security.mental_physical_harm": "security_background1",
    "security.drug_abuse": "security_background1",
    "security.arrest_conviction": "security_background2",
    "security.controlled_substance": "security_background2",
    "security.prostitution": "security_background2",
    "security.money_laundering": "security_background2",
    "security.trafficking_participation": "security_background2",
    "security.trafficking_assistance": "security_background2",
    "security.trafficking_benefit": "security_background2",
    "security.espionage_export": "security_background3",
    "security.terrorist_activity": "security_background3",
    "security.terrorist_support": "security_background3",
    "security.terrorist_membership": "security_background3",
    "security.terrorist_family": "security_background3",
    "security.genocide": "security_background3",
    "security.torture": "security_background3",
    "security.extrajudicial_violence": "security_background3",
    "security.child_soldiers": "security_background3",
    "security.religious_freedom": "security_background3",
    # Production CEAC renders both population-control questions at the end
    # of Part 3.  Keep this aligned with the working local V2 manifest; when
    # these were assigned to Part 4 they were never part of the live Part 3
    # plan and therefore remained visibly unanswered.
    "security.forced_abortion_sterilization": "security_background3",
    "security.forced_organ_transplant": "security_background3",
    "immigration.fraud_misrepresentation": "security_background4",
    "immigration.removal": "security_background4",
    "immigration.status_violation": "security_background4",
    "immigration.assisted_illegal_entry": "security_background4",
    "inadmissibility.child_custody": "security_background5",
    "inadmissibility.unlawful_voting": "security_background5",
    "inadmissibility.renounced_citizenship_tax": "security_background5",
    "inadmissibility.f1_public_school": "security_background5",
...
QUESTION_LABEL_TERMS = {
    ...
    "additional.specialized_skills": [
        ...
        "firearms explosives nuclear biological or chemical experience",
    ],
    "additional.military_service": ["ever served in the military"],
    "additional.paramilitary": [
        "served in been a member of or been involved with a paramilitary",
        "insurgent organization",
    ],
    "security.communicable_disease": [
        "communicable disease of public health significance",
    ],
    "security.mental_physical_harm": [
        "mental or physical disorder",
        "threat to the safety or welfare",
    ],
    "security.drug_abuse": ["drug abuser or addict"],
    "security.arrest_conviction": [
        "arrested or convicted for any offense or crime",
    ],
    "security.controlled_substance": [
        "conspiracy to violate",
        "law relating to controlled substances",
    ],
    "security.prostitution": [
        "engage in prostitution or unlawful commercialized vice",
        "past 10 years",
    ],
    "security.money_laundering": ["money laundering"],
    ...
    "security.trafficking_assistance": [
        "knowingly aided, abetted, assisted or colluded",
    ...
    "security.espionage_export": [
        "espionage, sabotage, export control violations",
    ],
    "security.terrorist_activity": ["engage in terrorist activities"],
    "security.terrorist_support": [
        "provide financial assistance or other support",
        "terrorists or terrorist organizations",
    ],
    "security.terrorist_membership": [
        "member or representative of a terrorist organization",
    ],
    "security.terrorist_family": [
        "spouse, son, or daughter",
        "engaged in terrorist activity",
        "last five years",
    ],
    "security.genocide": [
        "ordered, incited, committed, assisted",
    ...
    "security.extrajudicial_violence": [
        "extrajudicial killings, political killings",
        "other acts of violence",
    ],
    "security.child_soldiers": [
        "recruitment or the use of child soldiers",
    ],
    "security.religious_freedom": [
        "serving as a government official",
        "severe violations of religious freedom",
    ],
    "security.forced_abortion_sterilization": [
        "establishment or enforcement of population controls",
        "abortion against her free choice",
    ...
    "immigration.fraud_misrepresentation": [
        "obtain or assist others to obtain",
        "immigration benefit by fraud or willful misrepresentation",
    ],
    "immigration.removal": ["removed or deported from any country"],
    "immigration.status_violation": [
        "unlawfully present",
        "overstayed the amount of time granted",
        "violated the terms of a U.S. visa",
    ],
    "immigration.assisted_illegal_entry": [
        "assisted another person to enter or try to enter",
        "enter the United States illegally or through other unlawful means",
        "United States illegally",
    ],
    "inadmissibility.child_custody": [
        "withheld custody of a U.S. citizen child",
        "granted legal custody by a U.S. court",
    ],
    "inadmissibility.unlawful_voting": [
        "voted in the United States",
        "violation of any law or regulation",
    ],
    "inadmissibility.renounced_citizenship_tax": [
        "renounced United States citizenship",
        "purpose of avoiding taxation",
    ],
    "inadmissibility.f1_public_school": [
        "public elementary school on student (F) status",
...
QUESTION_CONTROL_HINTS = {
    ...
    "personal.permanent_resident_other_country": [
        "PermResOtherCntryInd", "OTH_RES", "PERM_RES",
    ],
    ...
    "contact.other_phones": ["rblAddPhone", "OTHER_PHONE", "ADDL_PHONE"],
    "contact.other_emails": ["rblAddEmail", "OTHER_EMAIL", "ADDL_EMAIL"],
    "contact.social_media": ["ddlSocialMedia"],
    "contact.other_platforms": ["rblAddSocial", "OTHER_SOCIAL", "OTHER_PLATFORM"],
    ...
    "security.communicable_disease": [
        "COMMUNICABLE_DISEASE", "PUBLIC_HEALTH_DISEASE", "DISEASE",
    ],
    "security.mental_physical_harm": [
        "MENTAL_PHYSICAL_DISORDER", "MENTAL_DISORDER", "PHYSICAL_DISORDER",
    ],
    "security.drug_abuse": [
        "DRUG_ABUSE", "DRUG_ADDICT", "SUBSTANCE_ABUSE",
    ],
    "security.arrest_conviction": [
        "ARREST_CONVICTION", "ARRESTED", "CONVICTED",
    ],
    "security.controlled_substance": [
        "CONTROLLED_SUBSTANCE", "DRUG_VIOLATION",
    ],
    "security.prostitution": ["PROSTITUTION"],
    "security.money_laundering": ["MONEY", "LAUNDER"],
    "security.trafficking_participation": ["HUMAN_TRAFFICKING", "TRAFFICKING_OFFENSE"],
    "security.trafficking_assistance": [
    ...
    "security.espionage_export": [
        "ESPIONAGE", "SABOTAGE", "EXPORT_CONTROL",
    ],
    "security.terrorist_activity": ["TERRORIST_ACTIVITY", "TERRORISM"],
    "security.terrorist_support": [
        "TERRORIST_SUPPORT", "TERRORIST_ASSISTANCE", "TERRORIST_ASSIST",
    ],
    "security.terrorist_membership": [
        "TERRORIST_MEMBERSHIP", "TERRORIST_ORGANIZATION",
        "TERRORIST_ORG", "TERRORIST_MEMBER",
    ],
    "security.terrorist_family": ["TERRORIST_FAMILY", "TERRORIST_RELATIVE"],
    "security.genocide": ["GENOCIDE"],
    "security.torture": ["TORTURE"],
    "security.extrajudicial_violence": [
        "EXTRAJUDICIAL", "POLITICAL_KILLING", "POLITICAL_KILL",
        "ACTS_OF_VIOLENCE",
    ],
    "security.child_soldiers": ["CHILD_SOLDIERS", "CHILD_SOLDIER"],
    "security.religious_freedom": [
        "RELIGIOUS_FREEDOM", "RELIGIOUS_VIOLATION",
    ],
    "security.forced_abortion_sterilization": [
        "FORCED_ABORTION", "FORCED_STERILIZATION", "POPULATION_CONTROL",
    ],
    "security.forced_organ_transplant": [
        "FORCED_TRANSPLANT", "ORGAN_TRANSPLANT", "BODILY_TISSUE",
    ],
    "immigration.fraud_misrepresentation": [
        "FRAUD_MISREPRESENTATION", "IMMIGRATION_FRAUD", "FRAUD",
        "MISREPRESENTATION",
    ],
    "immigration.removal": [
        "REMOVAL_DEPORTATION", "REMOVED", "DEPORTED", "REMOVAL", "DEPORT",
    ],
    "immigration.status_violation": [
        "STATUS_VIOLATION", "UNLAWFUL_PRESENCE", "OVERSTAY",
    ],
    "immigration.assisted_illegal_entry": [
        "ASSISTED_ILLEGAL_ENTRY", "ALIEN_SMUGGLING", "ILLEGAL_ENTRY",
    ],
    "inadmissibility.child_custody": ["CHILD_CUSTODY"],
    "inadmissibility.unlawful_voting": ["UNLAWFUL_VOTING", "VOTING"],
    "inadmissibility.renounced_citizenship_tax": ["RENOUNCED_CITIZENSHIP", "TAX"],
    "inadmissibility.f1_public_school": ["PUBLIC_SCHOOL", "F_STATUS"],
}
...
FIELD_CONTROL_HINTS = {
    ...
    "contact.primaryPhone": ["APP_HOME_TEL"],
    "contact.secondaryPhone": ["APP_MOBILE_TEL"],
    "contact.workPhone": ["APP_BUS_TEL"],
    "contact.email": ["APP_EMAIL_ADDR"],
}
...
def _simple_question_action(question, *, allow_sensitive=False):
    answer = _clean(question.get("answer"), 100).lower()
    if (
        not answer
        or answer == "unknown"
        or (question.get("sensitive") and not allow_sensitive)
    ):
        return None
    ...
    if answer_type == "yes_no" and answer not in {"yes", "no"}:
        return None
    if question.get("id") == "contact.social_media":
        # CEAC has a provider dropdown, not a Yes/No group. "yes" is
        # represented by the provider/handle records emitted separately.
        if answer != "no":
            return None
        return _action(
            "contact.social_media", "Social Media Provider/Platform",
            "select_text", "NONE", optionTerms=["NONE"],
            labelTerms=["Social Media Provider/Platform"],
            controlHints=["ddlSocialMedia"], causesRefresh=True,
        )
    ...
    explicit_hints = list(QUESTION_CONTROL_HINTS.get(question_id, []))
    if not explicit_hints:
        # Preserve a machine-readable CEAC suffix even when an intake
        # question has no hand-written alias yet.  The visible questionnaire
        # label can be localized (Chinese in production), while ASP.NET input
        # names remain English.  Without this fallback a visible required
        # radio group was incorrectly classified as conditional/absent and
        # Next became CEAC's first detector of the missing answer.
        semantic_tail = question_id.rsplit(".", 1)[-1].strip("_")
        if semantic_tail:
            normalized_tail = re.sub(
                r"[^A-Za-z0-9]+", "_", semantic_tail
            ).strip("_").upper()
            if len(normalized_tail) >= 4:
                explicit_hints = [
                    f"{normalized_tail}_IND",
                    normalized_tail,
                ]
    extra = {
        "labelTerms": [term for term in label_terms if term],
        "controlHints": explicit_hints,
    }
    if allow_sensitive and question.get("clientSubmitted") is True:
        extra["sourceAnswerExplicit"] = True
    kind = "yes_no"
...
def _field_action(fields, field_id, label, kind, terms, *, option_terms=None):
    ...
    if kind == "text" and compact_value in {
        "D", "DNA", "DOESNOTAPPLY", "DONOTKNOW", "NOTAPPLICABLE", "N/A", "NA",
    }:
        return _action(
...
def _us_contact_actions(fields, questions):
    ...
    if mode == "organization":
        _append(actions, _action(
            ...
            controlHints=["US_POC_NAME_NA"],
        ))
    for detail_id, field_id, label, kind, terms, hints in (
        ("surname", "contact.surname", "Contact Surnames", "text", ["Contact Person", "Surnames"], ["US_POC_SURNAME"]),
        ("givenNames", "contact.givenNames", "Contact Given Names", "text", ["Contact Person", "Given Names"], ["US_POC_GIVEN_NAME"]),
        ("organization", "contact.organizationName", "Organization Name", "text", ["Organization Name"], ["US_POC_ORGANIZATION"]),
        ("relationship", None, "Relationship to You", "select_text", ["Relationship to You"], ["US_POC_REL_TO_APP"]),
        ("phone", "contact.phone", "Phone Number", "text", ["Phone Number"], ["US_POC_HOME_TEL"]),
        ("email", "contact.usEmail", "Email Address", "text", ["Email Address"], ["US_POC_EMAIL_ADDR"]),
    ):
        ...
        if detail_id == "email" and is_does_not_apply_value(value):
            _append(actions, _action(
                "us_contact.email", "Email Address",
                "does_not_apply", "true",
                labelTerms=terms,
                controlHints=hints,
                checkboxTerms=["DOES NOT APPLY"],
            ))
            continue
        extra = {"optionTerms": [value]} if kind == "select_text" else {}
        _append(actions, _action(
            f"us_contact.{detail_id}", label, kind, value,
            labelTerms=terms, controlHints=hints, **extra
        ))
    if not _question_detail(question, "email", fields, "contact.usEmail"):
        _append(actions, _action(
            "us_contact.email", "Email Address",
            "does_not_apply", "true",
    ...
    address["street1"], address["street2"] = normalize_ceac_address_lines(
        address.get("street1"),
        "" if is_does_not_apply_value(address.get("street2"))
        else address.get("street2"),
    )
    for key, field_id, label, kind, terms, hints in (
        ("street1", "contact.usStreet1", "U.S. Contact Address Line 1", "text", ["Address", "Street Address (Line 1)"], ["US_POC_ADDR_LN1"]),
        ("street2", "contact.usStreet2", "U.S. Contact Address Line 2", "text", ["Address", "Street Address (Line 2)"], ["US_POC_ADDR_LN2"]),
        ("city", "contact.usCity", "U.S. Contact City", "text", ["Address", "City"], ["US_POC_ADDR_CITY"]),
        ("state", "contact.usState", "U.S. Contact State", "select_text", ["Address", "State"], ["US_POC_ADDR_STATE"]),
        ("postalCode", "contact.usPostalCode", "U.S. Contact ZIP Code", "text", ["Address", "ZIP Code"], ["US_POC_ADDR_POSTAL_CD"]),
    ):
        value = address.get(key) or _field_value(fields, field_id)
        if not value:
            continue
        extra = {"optionTerms": [value]} if kind == "select_text" else {}
        _append(actions, _action(
            f"us_contact.address.{key}", label, kind, value,
            labelTerms=terms, controlHints=hints, **extra
        ))
    return actions
...
def _work_actions(fields, questions):
    ...
    # The questionnaire may retain a short preview fragment while the
    # reviewed extracted field contains the complete employer address. Use
    # the most informative reviewed source instead of allowing the fragment
    # to hide the fuller value merely because it is non-empty.
    address_candidates = [
        _clean((question.get("originalDetails") or {}).get("address"), 500),
        _clean((question.get("details") or {}).get("address"), 500),
        _field_value(fields, "work.employerAddress"),
    ]
    raw_address = max(address_candidates, key=len, default="")
    if raw_address:
        structured = structure_address(raw_address, "CHINA")
        for action in _address_record_actions(
            "work.present.address", {
                "address": structured.get("line1") or raw_address,
                "line2": structured.get("line2"),
                "city": structured.get("city"),
                "region": structured.get("region"),
                "postalCode": structured.get("postalCode"),
                "country": structured.get("country") or "CHINA",
...
def _address_record_actions(
    prefix, record, index, label_prefix, hints, *, v2_record_semantics=False
):
    actions = []
    address = {
        "line1": _clean(record.get("address")),
        "line2": _clean(record.get("line2")),
    ...
    address["line1"], address["line2"] = normalize_ceac_address_lines(
    ...
    for key, label, kind, control_hints in (
        ...
        ("line2", f"{label_prefix} Street Address (Line 2)", "text", [*hints, "ADDR_LN2"]),
        ...
        value = address.get(key)
        if key == "postalCode" and not value:
            action_id = (
                f"{prefix}.record.{key}.{index}"
                if v2_record_semantics
                else f"{prefix}.{index}.{key}"
            )
            _append(actions, _action(
                action_id, label, "does_not_apply", "true",
    ...
    if education.get("answer") == "yes":
        ...
        for index, source_record in enumerate(education.get("records") or []):
            ...
            for key, label, kind, hints in (
                ...
                if value:
                    _append(actions, _action(
                        f"work.education.record.{key}.{index}",
                        label, kind, value,
                        labelTerms=[label], controlHints=hints, occurrence=index,
                    ))
            for action in _address_record_actions(
                "work.education", record, index, "School", ["EDUCATION"],
                v2_record_semantics=True,
            ):
                _append(actions, action)
    return actions
...
def build_browser_workflow(payload):
    ...
    Sensitive answers are included only when the client explicitly submitted
    an exact Yes/No response. Page navigation is permitted only after every
    listed action on that page is found and visibly verified; positive or
    unresolved sensitive branches remain manual-review boundaries.
    """
    fields = _field_map(payload)
    questions = _question_map(payload)
    ...
    for question in questions.values():
        ...
        if question.get("sensitive"):
            answer = _clean(question.get("answer"), 100).lower()
            explicitly_submitted = question.get("clientSubmitted") is True
            if explicitly_submitted and answer in {"yes", "no"}:
                _append(
                    pages[page_key],
                    _simple_question_action(
                        question,
                        allow_sensitive=True,
                    ),
                )
                # A submitted negative answer has no dependent explanation
                # branch.  It is safe to fill from the reviewed whitelist.
                if answer == "no":
                    continue
