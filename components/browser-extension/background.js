...
async function withDeadline(operation, milliseconds, onTimeout = () => {}) {
  let timer;
  try {
    return await Promise.race([
      ...
      new Promise((_, reject) => {
        timer = setTimeout(() => {
          onTimeout();
          reject(new Error("local_request_timeout"));
        }, milliseconds);
      })
    ]);
  } finally {
...
async function fetchLocal(url, options) {
  ...
  return withDeadline(
    fetch(url, { ...options, credentials: "omit", redirect: "error",
      headers: { ...options?.headers, "X-DocFlow-Extension-Id": chrome.runtime.id }, signal: controller.signal }),
    LOCAL_REQUEST_TIMEOUT_MS,
    () => controller.abort()
  );
}
const STORAGE_KEY = "docflowActiveTask";
const LOCAL_URL_PATTERNS = ["https://westoryvisa.com/*"];
...
const REVIEWED_NEGATIVE_HISTORY_IDS = new Set([
  "us_history.refusal_or_admission",
  "us_history.immigrant_petition"
]);

function taskActionIsAllowed(action) {
  const actionId = String(action?.id || "");
  if (!actionId || !action?.kind) return false;
  if (!SENSITIVE_ID_PATTERN.test(actionId)) return true;
  return (REVIEWED_NEGATIVE_HISTORY_IDS.has(actionId) || action.sourceAnswerExplicit === true)
    && action.kind === "yes_no"
    && String(action?.value || "").trim().toLowerCase() === "no";
}
...
function isLocalSender(sender) {
  try {
    ...
    return url.origin === "https://westoryvisa.com" && !url.username && !url.password;
  } catch (_error) {
    return false;
  }
}
...
function validateTaskUrl(rawUrl) {
  const url = new URL(String(rawUrl || ""));
  const validHost = url.origin === "https://westoryvisa.com" && !url.username && !url.password
    && !url.search && !url.hash;
  const validPath = /^\/api\/extension\/jobs\/codex-agent-[0-9a-f]{24}$/.test(url.pathname);
...
function validateTask(task) {
  ...
  if (workflowType === "ds160" && task.executor !== "chrome-extension-v1") {
    throw new Error("该任务不是 Chrome 插件执行计划");
  }
  const profile = taskProfile(task);
...
function isHardStopPage(task, rawUrl, title) {
  ...
  return /signandsubmit|sign and submit|electronic signature|payment|final submission/i.test(value);
}
...
async function notifyLocal(type, payload) {
  const tabs = await withDeadline(chrome.tabs.query({ url: LOCAL_URL_PATTERNS }), 1000)
    .catch(() => []);
  await Promise.all(tabs.map(async (tab) => {
    try {
      await withDeadline(chrome.tabs.sendMessage(tab.id, { type, ...payload }), 1000);
    } catch (_error) {
...
let remoteStatusSequence = 0;
async function postRemoteStatus(state, status, pageLabel) {
  remoteStatusSequence = Math.max(remoteStatusSequence + 1, Date.now());
  try {
    if (!state?.accessToken || !state.task?.statusUrl) throw new Error("missing_status_endpoint");
    const response = await fetchLocal(state.task.statusUrl, {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        ...
        observedRoutes: state.observedRoutes || []
        ,lastActionId: state.lastActionId || ""
        ,eventSequence: remoteStatusSequence
      })
    });
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    const acknowledgement = await withDeadline(response.json(), LOCAL_REQUEST_TIMEOUT_MS);
    if (acknowledgement.finished) {
      redactState(state, "completed", "已完成至照片页，任务资料已清除");
      state.armed = false;
      await setStoredState(state);
    }
    return true;
  } catch (_error) {
    // A failed status acknowledgement must really stop execution. Previously
    // only the UI said "paused" while the armed task kept sending fields.
    const current = await getStoredState();
    if (current?.active && current.task?.jobId === state.task?.jobId
      && current.eventSequence === state.eventSequence && current.armed) {
      current.armed = false;
      current.status = "blocked";
      current.statusCode = "local_sync_failed";
      current.message = "服务器未确认任务授权或进度，填写已暂停。请检查网络、登录和会员状态后重新准备任务。";
...
async function chooseCeacTab(preferredTabId = 0, preferStartPage = false) {
  ...
  if (preferStartPage) {
    ...
    // Reuse an existing CEAC tab even when it is on a CAPTCHA, security
    // question or other human checkpoint.  Opening a second start-page tab
    // loses the user's context and makes it look as if the task never began.
    if (startTabs.length > 1) {
      const activeStartTabs = startTabs.filter((tab) => tab.active);
      if (activeStartTabs.length === 1) return activeStartTabs[0];
      return startTabs.sort((left, right) => Number(right.lastAccessed || 0)
...
async function assignmentForTab(state, tab, options = {}) {
  if (!state?.active || !state.armed || !state.task || !tab?.url) return null;
  const page = pageForUrl(state.task, tab.url);
  if (!page) return null;
  const pageCompleted = Boolean(state.completedPages?.[page.key]);
  const recoveredDs160Page = taskWorkflowType(state.task) === "ds160" && pageCompleted;
  return {
    jobId: state.task.jobId,
    page,
    // DS-160 normal pages always run in continuous mode.  Page metadata,
    // required-field audits and the hard-stop policy still decide whether a
    // particular page may advance, but a stale UI toggle can no longer leave
    // a verified ordinary page waiting for a manual Next click.
    autoNext: taskWorkflowType(state.task) === "appointment" ? false : true,
    // CEAC does not preserve unsaved form values when a session is restored.
    // A route recorded as completed may therefore be a freshly empty page
    // after CAPTCHA / security recovery. DS-160 actions are idempotent, so
    // replay the page plan and audit it again instead of trusting the stale
    // completedPages marker. Appointment tasks retain their prior behavior.
    resumeState: recoveredDs160Page
      ? (options.manualContinue ? "manual_continue" : "session_recovery")
      : pageCompleted
...
async function startTask(message, sender) {
  if (!isLocalSender(sender)) throw new Error("只能从WestoryVisa 网站 启动任务");
  ...
  const response = await fetchLocal(taskUrl, {
    headers: { "Authorization": `Bearer ${accessToken}` },
    cache: "no-store"
  });
  const payload = await withDeadline(response.json(), LOCAL_REQUEST_TIMEOUT_MS);
  if (!response.ok) throw new Error(payload.error || `任务读取失败（HTTP ${response.status}）`);
  if (payload.statusUrl !== `${taskUrl}/status`) throw new Error("进度接口与任务来源不匹配");
  const task = validateTask(payload);
  const profile = taskProfile(task);
  const state = {
    active: true,
    // A DS-160 plugin task is armed as soon as it is paired.  Human-only
    // checkpoints remain unmapped and therefore receive no assignment; once
    // the user reaches the first mapped form page, filling starts without a
    // second popup click and remains armed across ordinary-page navigation.
    armed: false,
    task,
    taskUrl,
    accessToken,
    autoNext: profile.workflowType === "appointment" ? false : true,
  ...
  try {
    ...
    if (!tab) {
      if (profile.workflowType === "ds160") {
        state.armed = false;
        state.status = "waiting_for_entry";
        state.pageLabel = `${profile.label} start`;
        state.message = "没有找到已打开的 CEAC 标签页；请在当前 Chrome 中打开 DS-160，再点击开始填写";
        await setStoredState(state);
      ...
      if (!pageForUrl(task, tab.url) && tab.url !== task.targetUrl) {
        // Never force-navigate an existing CEAC form. CEAC deliberately warns
        // when a dirty form is abandoned, and replacing the tab here caused a
        // recurring "Leave site?" dialog. Keep the applicant's current page
        // intact and wait for a mapped form page instead.
        if (profile.workflowType !== "ds160"
          || !String(tab.url || "").startsWith("https://ceac.state.gov/GenNIV/")) {
...
async function resumeTask(sender) {
  if (!isLocalSender(sender) && !isExtensionSender(sender)) {
    throw new Error("只能从WestoryVisa 网站 或扩展面板开始识别页面");
  }
  ...
  if (!tab) {
    if (profile.workflowType === "ds160") {
      ...
      state.status = "waiting_for_entry";
      state.pageLabel = `${profile.label} start`;
      state.message = "没有找到已打开的 CEAC 标签页；插件不会另开页面，请先在当前 Chrome 中打开 DS-160";
      await setStoredState(state);
      await postRemoteStatus(state, "waiting_for_entry", state.pageLabel);
  ...
  await setStoredState(state);
  if (!await postRemoteStatus(state, "running", page.label)) return publicState(state);
...
async function pauseTask(sender) {
  if (!isLocalSender(sender) && !isExtensionSender(sender)) {
    throw new Error("只能从WestoryVisa 网站 或扩展面板暂停任务");
  }
  const state = await getStoredState();
  if (!state?.active || !state.task) return publicState(state);
  state.armed = false;
  state.status = "paused";
  state.statusCode = "user_paused";
  state.message = "任务已暂停，客户字段仍保留；点击“继续填写”即可从当前页面恢复";
  await setStoredState(state);
  const tabId = state.taskTabId || state.ceacTabId || 0;
  if (tabId) {
    try {
      await chrome.tabs.sendMessage(tabId, { type: "DOCFLOW_PAUSE_PAGE" });
    } catch (_error) {
      // The page may be navigating. The stored armed flag still prevents a
      // newly injected agent from resuming until the user explicitly starts.
    }
  }
  await notifyLocal("DOCFLOW_TASK_STATUS", publicState(state));
  return publicState(state);
...
async function updateFromPage(message, sender) {
  ...
  if (!state.armed || state.status === "paused") {
    return { ok: true, stopRequested: true };
  }
  const assignedTabId = state.taskTabId || state.ceacTabId;
  if (assignedTabId && assignedTabId !== sender.tab?.id) {
    return { ok: true, stopRequested: true };
  }
  ...
  // A recovered/redrawn page may lose values previously recorded as done.
  // Replace that page's verified set instead of keeping a monotonic union.
  const completed = new Set((state.completedActionIds || []).filter(actionId =>
    !Array.isArray(message.completedActionIds) || !allowedActionIds.has(actionId)));
  ...
  const lastActionId = String(message.lastActionId || "").slice(0, 160);
  state.lastActionId = allowedActionIds.has(lastActionId) ? lastActionId : "";
  ...
  await setStoredState(state);
  // Persist every action-level transition. The UI may summarize once a
  // minute, but the audit trail must not lose the steps between snapshots.
  state.lastRemoteSync = Date.now();
  await setStoredState(state);
  const synced = await postRemoteStatus(state, state.status, page.label);
  await notifyLocal("DOCFLOW_TASK_STATUS", publicState(state));
  const current = await getStoredState();
  return { ok: true, stopRequested: !synced || !current?.active || !current.armed
    || current.task?.jobId !== message.jobId };
}
...
async function handleCeacReady(message, sender) {
  ...
  const assignedTabId = state.taskTabId || state.ceacTabId;
  if (assignedTabId && assignedTabId !== sender.tab?.id) return { active: false };
  if (!taskAllowsUrl(state.task, sender.tab?.url)) {
    return { active: false };
  }
  const profile = taskProfile(state.task);
  ...
  const recoverableSafetyCodes = new Set([
    "application_error", "session_expired", "captcha", "security_block"
  ]);
...
async function stopTask(sender) {
  if (!isLocalSender(sender) && !isExtensionSender(sender)) {
    throw new Error("只能从WestoryVisa 网站 或扩展面板停止任务");
  }
  const state = await getStoredState();
  if (!state) return publicState(null);
  const taskTabId = state.taskTabId || state.ceacTabId || 0;
  if (taskTabId) {
    await chrome.tabs.sendMessage(taskTabId, { type: "DOCFLOW_PAUSE_PAGE" })
      .catch(() => {});
  }
  state.eventSequence = Number(state.eventSequence || 0) + 1;
  await postRemoteStatus(state, "revoked", state.pageLabel);
  state.active = false;
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  (async () => {
    ...
    if (message.type === "docflow.pauseTask") return pauseTask(sender);
