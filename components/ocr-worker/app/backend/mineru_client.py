...
def _upload_file(upload_url, path):
    ...
    connection = http.client.HTTPSConnection(
        parsed.hostname, parsed.port or 443, timeout=180
    )
    try:
        with file_path.open("rb") as opened:
            # MinerU's OSS URL is signed without Content-Type. urllib.request
            # adds application/x-www-form-urlencoded for byte bodies, which
            # changes OSS's canonical signature and produces HTTP 403.
            connection.request(
                "PUT",
                target,
                body=opened,
        ...
        error_body = response.read(2048)
        if response.status not in {200, 201, 204}:
            detail = _upload_error_code(error_body)
            suffix = f"：{detail}" if detail else ""
            raise MinerUError(
                f"MinerU 文件上传失败（HTTP {response.status}）{suffix}"
            )
    except MinerUError:
        raise
...
def _upload_error_code(body):
    """Return only OSS's safe machine-readable error code."""
    try:
        code = str(ET.fromstring(body).findtext("Code") or "").strip()
    except (ET.ParseError, TypeError, ValueError):
        return ""
    return code if code.replace("-", "").replace("_", "").isalnum() else ""
