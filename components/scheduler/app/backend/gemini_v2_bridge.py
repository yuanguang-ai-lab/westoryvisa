...
from .agent_client import (
    ...
    confirm_computer_use_form_entry,
...
def v2_runtime_readiness(base_url=None):
    ...
    browser_runtime = str(
        payload.get("browserRuntime") or "playwright_chromium"
    )
    human_first_ready = bool(
        browser_runtime != "chrome_stable_human_first"
        or (
            payload.get("humanFirstReady")
            and payload.get("chromeStableInstalled")
            and payload.get("cdpAttachReady")
        )
    )
    ready = bool(
        payload.get("ok")
    ...
    return {
        ...
        "humanFirstReady": human_first_ready,
        "message": (
            "Gemini V2 与 Linux 虚拟显示器已就绪"
            if ready else
            "官方 Chrome 人工先行接管环境尚未就绪"
            if not human_first_ready else str(
...
def confirm_provider_form_entry(provider_job_id, base_url=None):
    return confirm_computer_use_form_entry(
        provider_job_id, **_worker_target(base_url)
    )
...
def provider_status(provider):
    ...
    return {
        ...
        "browserRuntime": str(
            provider.get("browser_runtime") or "playwright_chromium"
        )[:80],
        "humanFirst": provider.get("human_first") is True,
    }
