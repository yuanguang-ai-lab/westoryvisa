...
function renderDraftPreview() {
  return `
    ...
    <button class="demo-primary-action" type="button" data-demo-go-stage="7">查看 DS-160 实际填写 →</button>`;
}
  ...
  return `
    ...
    <div class="computer-workspace">
      ...
      <aside class="computer-console">
        ...
        <button type="button" data-demo-go-stage="7">进入 DS-160 填写演示</button>
      </aside>
    </div>`;
}

function renderCeacFilling() {
  return `
    <div class="real-screen-toolbar"><div><span>⌂</span> 工作台</div><i>/</i><div>← DS-160 初稿</div><div class="ceac-view-label">逐页填写界面</div></div>
    <div class="ceac-demo-frame">
      <header class="ceac-header">
        <div class="ceac-seal" aria-hidden="true">US</div>
        <div><span>U.S. Department of State</span><strong>Consular Electronic Application Center</strong></div>
        <em>DS-160</em>
      </header>
      <nav class="ceac-nav" aria-label="DS-160 页面进度">
        <span class="done">Getting Started</span><span class="active">Personal</span><span>Address and Phone</span><span>Passport</span><span>Travel</span><span>Security</span>
      </nav>
      <div class="ceac-workspace">
        <main class="ceac-form">
          <div class="ceac-form-title"><div><small>PERSONAL INFORMATION 1</small><h3>Personal Information</h3></div><span>8 / 10 fields</span></div>
          <p class="ceac-instruction">Please provide the following information. All answers must be entered in English unless otherwise instructed.</p>
          <div class="ceac-fields">
            <label><span>Surnames</span><input value="W***" readonly aria-label="Surnames example value"><small>As shown in passport</small></label>
            <label><span>Given Names</span><input value="A***" readonly aria-label="Given Names example value"><small>As shown in passport</small></label>
            <label class="wide"><span>Full Name in Native Alphabet</span><input value="示例申请人" readonly aria-label="Native name example value"><small>已从初稿写入 · 等待人工复读</small></label>
            <label><span>Have you ever used other names?</span><select aria-label="Other names example value" disabled><option selected>NO</option></select></label>
            <label><span>Do you have a telecode?</span><select aria-label="Telecode example value" disabled><option selected>NO</option></select></label>
            <fieldset><legend>Sex</legend><label><input type="radio" checked disabled> Male</label><label><input type="radio" disabled> Female</label></fieldset>
            <label><span>Marital Status</span><select aria-label="Marital status example value" disabled><option selected>SINGLE</option></select></label>
            <label><span>Date of Birth</span><input value="19 AUG 2001" readonly aria-label="Date of birth example value"></label>
          </div>
          <div class="ceac-form-actions"><button type="button">Back</button><button type="button" class="save">Save</button><button type="button" class="next">Save and Continue</button></div>
        </main>
        <aside class="ceac-assist">
          <header><span>WESTORYVISA ASSIST</span><strong>正在填写当前页面</strong><i>80%</i></header>
          <div class="assist-progress"><span></span></div>
          <ol>
            <li class="done"><i>✓</i><div><strong>姓与名</strong><span>已写入并复读</span></div></li>
            <li class="done"><i>✓</i><div><strong>本国语言姓名</strong><span>已写入并复读</span></div></li>
            <li class="active"><i>3</i><div><strong>婚姻与出生信息</strong><span>正在核对页面结果</span></div></li>
            <li><i>4</i><div><strong>进入下一页</strong><span>核验无误后继续</span></div></li>
          </ol>
          <div class="assist-boundary"><strong>人工保留步骤</strong><span>验证码、敏感背景判断、电子签名和最终提交由申请人或授权顾问完成。</span></div>
          <a href="https://ceac.state.gov/GenNIV/Default.aspx" target="_blank" rel="noopener noreferrer" data-demo-external>打开官方 DS-160 页面 ↗</a>
        </aside>
      </div>
      <footer class="ceac-caption"><span>页面填写演示 · 使用脱敏示例数据</span><p>界面用于展示 WestoryVisa 的逐页辅助流程，并非美国政府网站或授权代表。</p></footer>
    </div>`;
}
...
function renderStage(stage, index) {
  ...
  if (index === 6) screen.innerHTML = demoStageMode === "computer" ? renderComputerUse() : renderDraftPreview();
  else if (index === 7) screen.innerHTML = renderCeacFilling();
  else screen.innerHTML = renderStandardStage(stage, index);
}
...
function initializeDemo() {
  ...
  player.addEventListener("click", (event) => {
    const allowedControl = event.target.closest("[data-demo-chapter], [data-demo-mode], [data-demo-go-stage], [data-demo-external], #demoToggle");
  ...
  const wireModeButtons = () => {
    ...
    player.querySelectorAll("[data-demo-go-stage]").forEach((button) => button.addEventListener("click", () => {
      wantsPlay = false;
      select(Number(button.dataset.demoGoStage));
    }));
  };
