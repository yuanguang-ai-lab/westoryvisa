const ANALYTICS_API = DocFlowApi.apiBaseUrl;
...
async function request(path, options = {}) {
  const response = await DocFlowApi.request(`${ANALYTICS_API}${path}`, {
    cache: "no-store",
...
async function loadSummary() {
  ...
  try {
    ...
    if (error.code === "unauthorized") {
      window.setTimeout(() => { window.location.href = "/workspace"; }, 1800);
    }
  } finally {
...
async function loadConfig() {
  ...
  try {
    const response = await DocFlowApi.request(`${ANALYTICS_API}/product/config`, { cache: "no-store" });
