(() => {
  ...
  const STORAGE_KEY = "westoryvisaCountry";
  ...
  const UI = {
    CN: {
      ...
      active: "当前版本",
      title: "选择你服务的国家",
      body: "页面语言和机构账号会固定到所选国家。",
      close: "关闭国家选择",
      locked: "机构账号已固定国家"
    },
    MX: {
      choose: "Elegir país de servicio",
      active: "Versión actual",
      title: "Elige el país donde presta servicio tu agencia",
      body: "El idioma del sitio y la cuenta de la agencia quedarán vinculados al país elegido.",
      close: "Cerrar selector de país",
      locked: "El país está fijado por la cuenta de la agencia"
    },
    BR: {
      choose: "Escolher país de atendimento",
      active: "Versão atual",
      title: "Escolha o país atendido pela sua agência",
      body: "O idioma do site e a conta da agência ficarão vinculados ao país escolhido.",
      close: "Fechar seletor de país",
      locked: "O país é definido pela conta da agência"
    },
    IN: {
      choose: "Choose service country",
      active: "Current version",
      title: "Choose the country your agency serves",
      body: "The site language and agency account will be tied to the selected country.",
      close: "Close country selector",
      locked: "Country is fixed by the agency account"
    }
  };

  // One source phrase, then natural Mexican Spanish, Brazilian Portuguese and Indian English.
  // Keeping the Chinese source intact lets the existing China interface remain untouched.
  const COPY = [
  ...
  function selectedCode() {
    const query = new URLSearchParams(window.location.search).get(PARAMETER);
    const candidate = String(query || "CN").toUpperCase();
    return COUNTRY_ORDER.includes(candidate) ? candidate : "CN";
  }
  ...
  function dynamicTranslation(value) {
    ...
    if (version) {
      ...
      return code === "MX" ? `Versión ${name}${version[2] ? ` · ${version[2]}` : ""}`
        : code === "BR" ? `Versão ${name}${version[2] ? ` · ${version[2]}` : ""}`
          : `${name} version${version[2] ? ` · ${version[2]}` : ""}`;
    }
  ...
  function translateElement(element) {
    ...
    attributes.forEach((name) => {
      ...
      const next = translateValue(originals[name]);
  ...
  function apply(root = document) {
    if (applying) return;
    applying = true;
    document.documentElement.lang = COUNTRIES[code].locale;
  ...
  function localisedHref(rawHref) {
    ...
    try {
      ...
      if (url.origin !== window.location.origin || url.searchParams.has("intake")) return href;
  ...
  function updateLinks(root = document) {
    ...
    links.forEach((link) => {
      if (link.dataset.countrySelect) return;
  ...
  function countryHref(nextCode) {
    const url = new URL(window.location.href);
    url.searchParams.set(PARAMETER, nextCode);
    return `${url.pathname}${url.search}${url.hash}`;
  }

  function selectionButton(country, { detailed = false } = {}) {
    const names = {
      CN: { CN: "中国", MX: "墨西哥", BR: "巴西", IN: "印度" },
      MX: { CN: "China", MX: "México", BR: "Brasil", IN: "India" },
      BR: { CN: "China", MX: "México", BR: "Brasil", IN: "Índia" },
      IN: { CN: "China", MX: "Mexico", BR: "Brazil", IN: "India" }
    }[code];
    const displayName = names[country.code];
    const languages = {
      CN: { CN: "简体中文", MX: "西班牙语", BR: "葡萄牙语", IN: "英语" },
      MX: { CN: "Chino simplificado", MX: "Español", BR: "Portugués", IN: "Inglés" },
      BR: { CN: "Chinês simplificado", MX: "Espanhol", BR: "Português", IN: "Inglês" },
      IN: { CN: "Simplified Chinese", MX: "Spanish", BR: "Portuguese", IN: "English" }
    }[code];
    const unavailable = locked && country.code !== code;
    const control = document.createElement(unavailable ? "button" : "a");
    if (control instanceof HTMLButtonElement) {
      control.type = "button";
      control.disabled = true;
    } else {
      control.href = countryHref(country.code);
      if (country.code === code) control.setAttribute("aria-current", "page");
    }
    control.className = `country-version-option${country.code === code ? " active" : ""}`;
    control.dataset.countrySelect = country.code;
    control.innerHTML = detailed
      ? `<span>${country.flag}</span><span><strong>${displayName}</strong><small>${languages[country.code]}</small></span>`
      : `<span>${country.flag}</span><span>${displayName}</span>`;
    return control;
  }
  ...
  function renderControls() {
    ...
    document.querySelectorAll("[data-country-current]").forEach((element) => {
      const label = `${COUNTRIES[code].flag} ${COUNTRIES[code].localName}`;
      if (element.textContent !== label) element.textContent = label;
      element.classList.add("active");
      element.title = locked ? copy.locked : copy.choose;
    });
    ...
    if (bar) {
      ...
      options.replaceChildren(...COUNTRY_ORDER.map((item) => selectionButton(COUNTRIES[item])));
    }
    const picker = document.querySelector(".country-picker");
    if (picker) {
      picker.querySelector("h2").textContent = copy.title;
      picker.querySelector("p").textContent = locked ? copy.locked : copy.body;
      ...
      picker.querySelector(".country-picker-grid").replaceChildren(
        ...COUNTRY_ORDER.map((item) => selectionButton(COUNTRIES[item], { detailed: true }))
      );
    }
  }
  ...
  function setUrlCountry(nextCode) {
    window.history.replaceState({}, "", countryHref(nextCode));
  }

  function activate(nextCode, options = {}) {
    const normalized = String(nextCode || "").toUpperCase();
    if (!COUNTRY_ORDER.includes(normalized) || (locked && normalized !== code)) return false;
  ...
  const api = {
    ...
    get locale() { return COUNTRIES[code].locale; },
    get country() { return { ...COUNTRIES[code] }; },
    activate,
    apply,
    lock,
    unlock,
