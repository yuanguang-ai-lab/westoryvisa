...
- `audit-regressions.cjs`：72 项语义回归检查。覆盖 4 种显示语言 × 4 个机构服务国家的登录/注册请求体和请求头；语言切换不得改变服务国家；国家选择保留已输入的表单数据；MX 档案按当前语言显示提示；普通账号国家锁和案例过滤、平台管理员国家切换入口、插件仅支持中国版的既有范围均保持。另验证文件协议打开、API 版本过旧、后端断开这 3 类提示的四语言准确输出和切回中文；加载真实 workspace.html 及全部脚本、执行完整 boot，分别模拟延迟 HTTP 404、网络失败、API 20 响应，检查最终真实登录界面的提示语言及预览图的三条标题确实随语言切换。资源和接口均在本地拦截，不连接真实后端。

测试读取 `components/frontend/www/`。全套共 **166 项**（38 项静态/优先级/往返、52 项动态界面、4 项 API 版本保护、72 项业务语义回归）。
